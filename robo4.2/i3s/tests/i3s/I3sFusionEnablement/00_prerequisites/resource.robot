...
Update Planscript Partial Names With Full Names
    [Documentation]    Update partial names of planscripts to valid full names in the planscript URI of buildplans
    [Arguments]    ${bp}

    ${ps_full_name_list} =    Create List
    ${ps_list} =    I3S API GET PLAN SCRIPTS

    #Get all the planscripts and append to list
    :FOR  ${ps}    IN    @{ps_list['members']}
    \    Append To List     ${ps_full_name_list}    ${ps['name']}

    # Get buildsteps whose planscript full name has to be found
    ${buildSteps} =    Set Variable    ${bp['buildStep']}
    ${bp_with_ps_full_name} =    Create List

    :For    ${buildStep}    IN    @{buildSteps}
    \    ${ps_full_name} =     Get Plan Script Full Name By Partial Name    ${buildStep['planScriptUri']}    ${ps_full_name_list}
    \    Set To Dictionary     ${buildStep}    planScriptUri=${ps_full_name}
    \    Append To List   ${bp_with_ps_full_name}    ${buildStep}
    Set To Dictionary    ${bp}    buildStep=${bp_with_ps_full_name}
    [Return]    ${bp}

Get Plan Script Full Name By Partial Name
    [Documentation]    Get Plan Script Full Name By Partial Name
    [Arguments]    ${ps_partial_name}    ${ps_full_name_list}

    :For   ${ps_full_name}    IN    @{ps_full_name_list}
    \    ${bln_contains} =  Evaluate   "${ps_partial_name}" in "${ps_full_name}"
    \    Run keyword If    '${bln_contains}' == 'True'    Exit For Loop
