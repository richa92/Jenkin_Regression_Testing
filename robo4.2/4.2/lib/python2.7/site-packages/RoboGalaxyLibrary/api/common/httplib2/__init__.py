...
def has_timeout(timeout):  # python 2.6
    if hasattr(socket, '_GLOBAL_DEFAULT_TIMEOUT'):
        return (
            timeout is not None and timeout is not socket._GLOBAL_DEFAULT_TIMEOUT)
...
# A request will be tried 'RETRIES' times if it fails at the
...
def _entry_disposition(response_headers, request_headers):
    ...
    if 'pragma' in request_headers and request_headers[
            'pragma'].lower().find('no-cache') != -1:
        retval = "TRANSPARENT"
...
def _cnonce():
    dig = _md5(
        ...
        (time.ctime(), [
            "0123456789"[
                random.randrange(
                    0, 9)] for i in range(20)])).hexdigest()
    return dig[:16]
...
def _wsse_username_token(cnonce, iso_now, password):
    return base64.b64encode(
        _sha(
            "%s%s%s" %
            (cnonce, iso_now, password)).digest()).strip()
...
class DigestAuthentication(Authentication):
    ...
    def __init__(
        ...
        self.challenge['qop'] = (
            'auth' in [
                x.strip() for x in qop.split()]) and 'auth' or None
    ...
    def request(self, method, request_uri, headers, content, cnonce=None):
        ...
        if self.challenge.get('opaque'):
            headers[
                'authorization'] += ', opaque="%s"' % self.challenge['opaque']
        self.challenge['nc'] += 1
...
class GoogleLoginAuthentication(Authentication):
    ...
    def __init__(
        ...
        resp, content = self.http.request(
            "https://www.google.com/accounts/ClientLogin", method="POST", body=urlencode(auth), headers={
...
class Http(object):
    ...
    def _request(
        ...
        auths = [
            (auth.depth(request_uri),
             auth) for auth in self.authorizations if auth.inscope(
                host,
                request_uri)]
        ...
        if (self.follow_all_redirects or (
            ...
            if self.follow_redirects and response.status in [
                ...
                if redirections:
                    ...
                    if response.status == 301 and method in ["GET", "HEAD"]:
                        ...
                        _updateCache(
                            headers, response, content, self.cache, cachekey)
...
# including all socket.* and httplib.* exceptions.
    ...
    def request(
        ...
        try:
            ...
            if len(domain_port) == 2 and domain_port[
                    1] == '443' and scheme == 'http':
                scheme = 'https'
            ...
            if cached_value and method in [
                    "GET",
                    "HEAD"] and self.cache and 'range' not in headers:
                if '-x-permanent-redirect-url' in info:
                    ...
                    if entry_disposition == "STALE":
                        ...
                        if 'last-modified' in info and 'last-modified' not in headers:
                            headers[
                                'if-modified-since'] = info['last-modified']
                    elif entry_disposition == "TRANSPARENT":
                        pass

                    (response,
                     new_content) = self._request(conn,
                                                  authority,
                                                  uri,
                                                  request_uri,
                                                  method,
