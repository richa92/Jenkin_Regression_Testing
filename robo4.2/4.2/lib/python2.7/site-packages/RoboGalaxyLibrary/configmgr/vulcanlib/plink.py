"""
Contains the PLINKconnector class which runs external commands through the plink executable.
...
class PLINKconnector(Connector):

    """A connector which runs external commands on a remote machine through the plink executable.

    Plink is an executable distributed by the same team that creates PuTTY and can be downloaded
    from the same website.

    Of course, the plink executable should be installed on the local machine and be placed
    in the PATH.
    """

    def __init__(self, client=None, login=None, password=None):
        """Create a connector that runs commands on the specified client through plink.

        The client argument should be a hostname or IP address of the remote machine where
        the command will run. The login and password are required arguments.
        """
        ...
        if client is None or login is None or password is None:
            _LOG.error(
                "sorry, you asked for a plink connector but you don't have enough info")
            _LOG.error("plink connectors require, client, login and password")
            sys.exit(1)

    def send_cmd(self, cmd):
        """Run the specified command on the remote machine through plink in the foreground.
        ...
        """
        cmd = "plink -l " + self.login + " -pw " + \
            self.password + " " + self.client + " " + cmd
