...
from RoboGalaxyLibrary.netconf.ncclient import NCClientError
...
qualify = lambda tag, ns=BASE_NS_1_0: tag if ns is None else "{%s}%s" % (
    ns, tag)
...
def to_xml(ele, encoding="UTF-8"):
    ...
    return xml if xml.startswith(
        '<?xml') else '<?xml version="1.0" encoding="%s"?>%s' % (encoding, xml)
...
new_ele = lambda tag, attrs={
}, **extra: ET.Element(qualify(tag), attrs, **extra)

sub_ele = lambda parent, tag, attrs={
}, **extra: ET.SubElement(parent, qualify(tag), attrs, **extra)
