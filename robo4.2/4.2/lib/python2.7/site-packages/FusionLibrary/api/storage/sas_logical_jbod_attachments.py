...
class SasLogicalJbodAttachments(object):
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/sas-logical-jbod-attachments%s' % (self.fusion_client._host, param)
    ...
    def post(self, body, api=None, headers=None):
        if api:
            ...
            headers = self.fusion_client._headers.copy()
        uri = 'https://%s/rest/sas-logical-jbod-attachments' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('SAS Logical JBOD Attachment %s does not exist' % (name), level='WARN')
                return
