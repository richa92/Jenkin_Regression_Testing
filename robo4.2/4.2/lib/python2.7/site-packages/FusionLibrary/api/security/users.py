...
class User(object):
    ...
    def create(self, body, api=None, headers=None):
        ...
        uri = 'https://%s/rest/users' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None, watchTask=True):
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
            headers = self.fusion_client._headers.copy()
        if uri:
            ...
            if response['count'] == 0:
                logger._log('User %s does not exist' % (name), level='WARN')
                return
    ...
    def change_administrator_password(self, host, body, api=None, headers=None):
        ...
        uri = 'https://%s/rest/users/changePassword' % (host)
        response = self.fusion_client.post(uri=uri, headers=headers, body=json.dumps(body))
        return response
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/users/%s' % (self.fusion_client._host, param)
