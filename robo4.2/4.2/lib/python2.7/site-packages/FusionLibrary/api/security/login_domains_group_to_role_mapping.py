...
class LoginDomainsGroupToRoleMapping(object):
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Group %s does not exist' % (name), level='WARN')
                return
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/logindomains/grouptorolemapping%s' % (self.fusion_client._host, param)
    ...
    def validate(self, body, api=None, headers=None):
        if api:
            ...
            headers = self.fusion_client._headers.copy()
        uri = 'https://%s/rest/logindomains/grouptorolemapping/validator' % (self.fusion_client._host)
        response = self.fusion_client.post(uri=uri, headers=headers, body=json.dumps(body))
        return response
