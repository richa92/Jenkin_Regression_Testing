...
class LoginDomainsLoginCertificates(object):
    ...
    def get(self, api=None, headers=None, param=''):
        ...
        uri = 'https://%s/rest/logindomains/logincertificates%s' % (self.fusion_client._host, param)
        response = self.fusion_client.get(uri=uri, headers=headers)
        return response

    def update(self, body, param='', api=None, headers=None):
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
            headers = self.fusion_client._headers.copy()
