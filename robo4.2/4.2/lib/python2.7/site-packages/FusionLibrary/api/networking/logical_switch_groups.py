...
class LogicalSwitchGroup(object):
    ...
    def create(self, body, api=None, headers=None):
        ...
        uri = 'https://%s/rest/logical-switch-groups' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Logical Switch Group %s does not exist' % (name), level='WARN')
                return
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ''' add param=/defaultSettings, /settings/{settingsId} in keywords'''
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
            headers = self.fusion_client._headers.copy()
        if uri:
            ...
            uri = 'https://%s/rest/logical-switch-groups%s' % (self.fusion_client._host, param)
