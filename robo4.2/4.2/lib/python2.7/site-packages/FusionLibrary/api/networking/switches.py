...
class Switch(object):
    ...
    def create(self, body=None, uri=None, api=None, headers=None):
        if api:
        ...
        if uri:
            ...
            uri = 'https://%s/rest/switches' % (self.fusion_client._host)
        response = self.fusion_client.post(uri=uri, headers=headers, body=json.dumps(body))
        return response

    def refresh(self, uri, api=None, headers=None):
        # this method allows for a URI to be passed.  Providing a URI to the POST with no body
    ...
    def update(self, body, uri, api=None, headers=None, param=''):
            ...
            if uri:
                ...
                uri = 'https://%s/rest/switches%s' % (self.fusion_client._host, param)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Switch %s does not exist' % (name), level='WARN')
                return
