...
class DeployedTargets(object):
    ...
    def post(self, body, api=None, headers=None):
        if api:
            ...
            headers = self.fusion_client._headers
        uri = 'https://%s/rest/deployed-targets' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Deployed Target %s does not exist' % (name), level='WARN')
                return
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/deployed-targets%s' % (self.fusion_client._host, param)
