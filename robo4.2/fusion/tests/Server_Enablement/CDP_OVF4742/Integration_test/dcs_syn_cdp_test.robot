*** Settings ***
Documentation                   CDP integration test for DCS Synergy servers 
...
*** Variables ***
${APPLIANCE_IP}     16.83.15.219

*** Test Cases ***

Test case1 Create the server profile with default boot and Bios setting (optimised UEFI)
    [Documentation]   Test case to create a server profile with default boot and Bios setting (optimised UEFI)
    [Tags]     Createsp    Integrate_1        Integrate_2
    Create Server profile        ${syn_Server_profile}

Test case2 Power on Server
    [Documentation]   Test case to power on the server hardware
    [Tags]        Poweron        Integrate_1        Integrate_2
    Power on the Server        ${syn_server_ilo_hostname}

Test case3 Edit the server profile ( Legacy Bios editing the bios settings)
    [Documentation]   Test case to edit a server profile to Legacy Bios 
    [Tags]    editsp        Integrate_1        Integrate_2
    Edit the server profile        ${syn_Server_edit_profile}

Test case4 Edit the server profile and flash firmware
    [Documentation]   Test case to edit a server profile with firmware update with default bios setting
    [Tags]    firmware    Integrate_2
    Edit the server profile        ${syn_Server_edit_profile_with_firmware}

Test case5 Power off Server
    [Documentation]   Test case to power off the server hardware
    [Tags]        Poweroff        Integrate_1        Integrate_2
    Power off the Server        ${syn_server_ilo_hostname}

Test case6 Delete server profile
    [Documentation]   Test case to delete a server profile
    [Tags]        Deletesp        Integrate_1        Integrate_2
    Delete Server profile        ${syn_Server_profile}

*** Keywords ***
Setup
    [Documentation]    Setup to log in to synergy appliance
    Set Log Level  DEBUG
