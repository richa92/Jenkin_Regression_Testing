*** Settings ***
Documentation    CDP - Profile operation on DCS Rack servers
...
Create assigned server profile with overridden BIOS settings
    ...
    \    Power on Server    ${host_names[${index}]}
    \    Sleep    5m
    \    ${profile_get} =     Fusion Api Get Server Profiles        param=?filter="'name'=='${SP_3s[${index}][0]['name']}'"
    \    ${diff_list}=    Compare BIOS values in profile and input    ${SP_3s[${index}]}    ${profile_get}
