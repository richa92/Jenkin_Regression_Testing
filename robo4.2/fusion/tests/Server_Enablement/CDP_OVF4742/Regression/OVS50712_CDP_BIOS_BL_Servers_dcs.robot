*** Settings ***
Documentation    CDP - Profile operation on DCS C7K servers
...
Library            json
Library            Process
Suite Setup        Set up for c7k
Suite Teardown    Tear down set up
Resource        resources.robot
Resource        ../../../../Resources/api/fusion_api_resource.txt
Variables        OVS50712_CDP_BIOS_Datafile_dcs.py
...
Power off server
    ...
    :FOR    ${host_name}    in    @{c7k_host_names}
...
Add server profile with boot mode
    ...
    \    ${task}=    Run keyword if    '${r[0]['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
    ...
    \    Run keyword if    '${r['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10

Create Server Profiles with Server Hardware
    [Documentation]    Create Server Profiles with Server Hardware
    Remove All Alerts
    ${SP_1s_len} =    Get Length    ${c7k_SP_1s}
    :FOR   ${index}  in range  0   ${SP_1s_len}
    \    ${SP_assign}=   Get Variable Value      ${c7k_SP_1s[${index}]}
    \    Set to dictionary   ${SP_assign[0]}    serverHardwareUri    SH:${c7k_host_names[${index}]}
    \    ${r}=    Add Non Existing Server Profiles    ${SP_assign}
    \    ${task}=    Run keyword if    '${r[0]['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
    \    ${resp} =     Fusion Api Get Server Profiles        param=?filter="'name'=='${c7k_SP_1s[${index}][0]['name']}'"
    \    Return From Keyword If  ${resp['count']}==0  FAIL    Server profile creation failed
...
Un-assign server profile
    ...
    \    Run keyword if    '${resp['status_code']}'=='202'    Wait for task2    ${resp}    timeout=3600  interval=10
...
Remove server profile
    ...
    ${SP_1s_len} =    Get Length    ${c7k_SP_1s}
    :FOR   ${index}  in range  0   ${SP_1s_len}
    \    Power off Server    ${c7k_host_names[${index}]}
    \    ${r}=    Remove Server Profile  ${c7k_SP_1s[${index}][0]}  force=${True}
    \    Run keyword if    '${r['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
    \    ${resp} =     Fusion Api Get Server Profiles        param=?filter="'name'=='${c7k_SP_1s[${index}][0]['name']}'"
    \    Return From Keyword If  ${resp['count']}!=0  FAIL    Server profile creation failed
...
Edit profile and assign it to Server
    ...
    Remove All Alerts
    ${SP_2s_len} =    Get Length    ${c7k_SP_2s}
    :FOR   ${index}  in range  0   ${SP_2s_len}
    \    ${SP_assign}=   Get Variable Value      ${c7k_SP_2s[${index}]}
    \    Set to dictionary   ${SP_assign[0]}    serverHardwareUri    None
    \    ${resp}=    Edit Server Profile     ${SP_assign[0]}
    \    Run keyword if    '${resp['status_code']}'=='202'    Wait for task2    ${resp}    timeout=3600  interval=10
    \    Set to dictionary   ${SP_assign[0]}    serverHardwareUri    SH:${c7k_host_names[${index}]}
    \    ${resp}=    Edit Server Profile     ${SP_assign[0]}
    \    Run keyword if    '${resp['status_code']}'=='202'    Wait for task2    ${resp}    timeout=3600  interval=10
    \    ${r}=    Remove Server Profile  ${c7k_SP_2s[${index}][0]}  force=${True}
    \    Run keyword if    '${r['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
...
Create assigned profile with default BIOS overiridden settings
    ...
    \    ${r}=    Add Non Existing Server Profiles    ${SP_assign}
    \    ${task}=    Run keyword if    '${r[0]['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
    \    ${resp} =     Fusion Api Get Server Profiles        param=?filter="'name'=='${c7k_SP_2s[${index}][0]['name']}'"
    \    Return From Keyword If  ${resp['count']}==0  FAIL    Server profile creation failed
    \    ${r}=    Remove Server Profile  ${c7k_SP_2s[${index}][0]}  force=${True}
    \    Run keyword if    '${r['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
...
Create assigned server profile with overridden BIOS settings
    ...
    Remove All Alerts
    ${SP_3s_len} =    Get Length    ${c7k_SP_3s}
    :FOR   ${index}  in range  0   ${SP_3s_len}
    \    ${r}=    Add Non Existing Server Profiles    ${c7k_SP_3s[${index}]}
    \    ${task}=    Run keyword if    '${r[0]['status_code']}'=='202'    Wait for task2    ${r}    timeout=3600  interval=10
    \    Power on Server    ${c7k_host_names[${index}]}
    \    Power off Server    ${c7k_host_names[${index}]}
    \    Power on Server    ${c7k_host_names[${index}]}
    \    Sleep    5m
    \    ${profile_get} =     Fusion Api Get Server Profiles        param=?filter="'name'=='${c7k_SP_3s[${index}][0]['name']}'"
    \    ${diff_list}=    Compare BIOS values in profile and input    ${c7k_SP_3s[${index}]}    ${profile_get}
    \    Log    ${diff_list}    console=True
    \    ${len}=    Get Length    ${diff_list}
    \    Run keyword if    ${len}!=0    FAIL    Bios settings in profile are different.
