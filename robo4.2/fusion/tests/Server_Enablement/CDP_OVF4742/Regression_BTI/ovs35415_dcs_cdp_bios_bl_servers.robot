*** Settings ***
Documentation                  BL Server profiles and templates for BIOS TESTING.
...
OVS35415 - Create Assigned Profiles with BIOS Settings
    ...
    [TAGS]    BIOSOnly
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_bios_only}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    Power on Server and check if its ON     ${bl_server_ilo_hostname}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create Assigned Profiles with - F/W and BIOS Settings
    ...
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${dict} =       Add Firmware Baseline Uri To Dict      ${bl_${gen}_server_profile_firmware_and_bios['firmware']}
    Set to Dictionary    ${bl_${gen}_server_profile_firmware_and_bios}    firmware        ${dict}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_firmware_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create Assigned Profiles with - specific attributes
    ...
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_create_specific_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_specific_bios}
    Log     ${prof_list}
    Edit Server Profiles       ${prof_list}
    Delete Server Profiles     ${prof_list}
...
OVS35415 - Create Assigned Profiles with - Server Profile Template and Check if it is Compliant
    ...
    [TAGS]      SPT
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${templt_lst} =    Add Sht Name to dict     ${bl_${gen}_profile_templates[0]}
    Create Server Profile Template      ${templt_lst}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_with_template_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create Assigned Profiles - with selective re-apply on server hardware
    ...
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_bootmode_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create UnAssigned Profile with Default BIOS and Assign it to server
    ...
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    Log     ${prof_lst}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_unassign_defbios}
    Log     ${prof_list}
    Create Server Profiles Unassigned     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_assign_defbios}
    Log     ${prof_list}
    Power Off Server     ${bl_server_ilo_hostname}
    ${power_state}=    Get Server Power    ${bl_server_ilo_hostname}
...
OVS35415 - Create Assigned Profile with Default BIOS and Edit it
    ...
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_assign_defbios}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_assign_defbios}
    Edit Server Profiles       ${prof_list}
    Delete Server Profiles     ${prof_list}
...
OVS35415 - Create Assigned Profiles - with selective re-apply on profile connection
    ...
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_connection_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create UnAssigned Profiles and Verify
    [Documentation]    Testing UnAssigned Profiles
    [TAGS]    UnAssignedProfile
    ${gen}=    Convert to Lowercase    ${bl_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${bl_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_unassign}
    Create Server Profiles Unassigned     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${bl_${gen}_server_profile_assign}
...
Add Sht Name to dict
    ...
    ${sht_name} =   Get Sht Name    ${bl_server_ilo_hostname}
...
Add Firmware Baseline Uri To Dict
    [Documentation]     Adds/Updates the key/value {"firmwareBaselineUri":"/rest/firmwareBaselineUri/..."} to the given dictionary and returns a list
    [Arguments]     ${given_dict}
    ${id}=     Open Connection     ${bl_fusion_ip}
    ${login_output}=     Login    ${bl_fusion_ssh_credentials_username}    ${bl_fusion_ssh_credentials_password}
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${bl_fusion_admin_credentials_username}", "password":"${bl_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${bl_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ${fw_details} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${bl_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${bl_fusion_ip}/rest/firmware-drivers"
    ...
    \   Log     ${new_dict}
    \   Run Keyword If      "${fw_iso}" == "${bl_firmware_iso_uploaded_into_OneView}"      Set to Dictionary    ${dict}      firmwareBaselineUri      ${fw_uri}
...
Get Sht Name
    ...
    [Arguments]     ${hostname}
    ${id}=     Open Connection     ${bl_fusion_ip}
    ${output}=     Login    ${bl_fusion_ssh_credentials_username}    ${bl_fusion_ssh_credentials_password}
    ${shturi} =       Get Server Hardware Type URI By Server Hardware       ${hostname}
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${bl_fusion_admin_credentials_username}", "password":"${bl_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${bl_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ${sht_dict} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${bl_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${bl_fusion_ip}${shturi}"
...
Check Profile if NonCompliant with Template
    ...
    ${ProfDetails} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${bl_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${bl_fusion_ip}${svr_prof_uri}"
