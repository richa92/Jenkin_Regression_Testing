*** Settings ***
Documentation                   Rack Server profiles and templates for BIOS TESTING.
...
${DATA_FILE}        ./ovs35415_cdp_bios_data.py

*** Test Cases ***
OVS35415 - Create Assigned Profiles with BIOS Settings
    [Documentation]    Testing the Assigned Profiles with BIOS Settings
    [TAGS]    BIOSOnly
    ${gen} =    Convert to Lowercase    ${rack_server_genration}
    Add server as managed      ${rack_server_add}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_bios_only}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    Power on Server and check if its ON     ${rack_server_ilo_hostname}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create Assigned Profiles with - F/W and BIOS Settings
    ...
    ${gen}=    Convert to Lowercase    ${rack_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${dict} =       Add Firmware Baseline Uri To Dict      ${rack_${gen}_server_profile_firmware_and_bios['firmware']}
    Set to Dictionary    ${rack_${gen}_server_profile_firmware_and_bios}    firmware        ${dict}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_firmware_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create Assigned Profiles with - specific attributes
    ...
    ${gen}=    Convert to Lowercase    ${rack_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_create_specific_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_specific_bios}
    Log     ${prof_list}
    Edit Server Profiles       ${prof_list}
    Delete Server Profiles     ${prof_list}
...
OVS35415 - Create Assigned Profiles with - Server Profile Template and Check if it is Compliant
    ...
    [TAGS]      SPT
    ${gen}=    Convert to Lowercase    ${rack_server_genration}
    ${templt_lst} =    Add Sht Name to dict     ${rack_${gen}_profile_templates[0]}
    Create Server Profile Template      ${templt_lst}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_with_template_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create Assigned Profiles - with selective re-apply on server hardware
    ...
    ${gen}=    Convert to Lowercase    ${rack_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_bootmode_and_bios}
    Log     ${prof_list}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_bios_only}
    Log     ${prof_list}
...
OVS35415 - Create UnAssigned Profile with Default BIOS and Assign it to server
    ...
    Create Server Profile Template      ${templt_lst}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_unassign_defbios}
    Log     ${prof_list}
    Create Server Profiles Unassigned     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_assign_defbios}
    Log     ${prof_list}
    Power Off Server     ${rack_server_ilo_hostname}
    ${power_state}=    Get Server Power    ${rack_server_ilo_hostname}
...
OVS35415 - Create Assigned Profile with Default BIOS and Edit it
    ...
    ${gen}=    Convert to Lowercase    ${rack_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_assign_defbios}
    Create Server Profiles     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_edit_assign_defbios}
    Edit Server Profiles       ${prof_list}
    Delete Server Profiles     ${prof_list}
...
OVS35415 - Create UnAssigned Profiles
    ...
    ${gen}=    Convert to Lowercase    ${rack_server_genration}
    ${prof_lst} =       Add Sht Name to dict     ${rack_${gen}_server_profiles[0]}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_unassign}
    Create Server Profiles Unassigned     ${prof_list}
    ${prof_list} =    Add dicts to list    ${prof_lst[0]}    ${rack_${gen}_server_profile_assign}
...
Add Sht Name to dict
    ...
    ${sht_name} =   Get Sht Name    ${rack_server_ilo_hostname}
...
Add Firmware Baseline Uri To Dict
    ...
    ${login_output}=     Login    ${rack_fusion_ssh_credentials_username}    ${rack_fusion_ssh_credentials_password}
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${rack_fusion_admin_credentials_username}", "password":"${rack_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${rack_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ${fw_details} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${rack_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${rack_fusion_ip}/rest/firmware-drivers"
    ...
    \   Log     ${new_dict}
    \   Run Keyword If      "${fw_iso}" == "${rack_firmware_iso_uploaded_into_OneView}"      Set to Dictionary    ${dict}      firmwareBaselineUri      ${fw_uri}
...
Get Sht Name
    ...
    [Arguments]     ${hostname}
    ${id}=     Open Connection     ${rack_fusion_ip}
    ${output}=     Login    ${rack_fusion_ssh_credentials_username}    ${rack_fusion_ssh_credentials_password}
    ${shturi} =       Get Server Hardware Type URI By Server Hardware       ${hostname}
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${rack_fusion_admin_credentials_username}", "password":"${rack_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${rack_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ${sht_dict} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${rack_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${rack_fusion_ip}${shturi}"
    &{sht_dict} =     string to dictionary     ${sht_dict}
    ${sht_name}=    Get From Dictionary     ${sht_dict}    name
    [Return]    ${sht_name}
    Close Connection

Add{prof_list}er as managed
    [Documentation]    Add server as managed
    [Arguments]     ${servers}
...
Check Profile if NonCompliant with Template
    ...
    [Arguments]     ${profiles}
    ${id}=     Open Connection     ${rack_fusion_ip}
    ${Id}=     Login    ${rack_fusion_ssh_credentials_username}    ${rack_fusion_ssh_credentials_password}
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${rack_fusion_admin_credentials_username}", "password":"${rack_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${rack_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ...
    ${ProfDetails} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${rack_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${rack_fusion_ip}${svr_prof_uri}"
