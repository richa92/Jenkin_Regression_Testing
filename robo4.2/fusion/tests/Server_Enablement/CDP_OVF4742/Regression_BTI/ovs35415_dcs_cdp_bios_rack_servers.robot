...
Add Firmware Baseline Uri To Dict
    ...
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${rack_fusion_admin_credentials_username}", "password":"${rack_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${bl_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ${fw_details} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${rack_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${bl_fusion_ip}/rest/firmware-drivers"
...
Get Sht Name
    ...
    ${output}=    Execute Command     /usr/bin/curl -k -s -S -X POST -d '{"userName":"${rack_fusion_admin_credentials_username}", "password":"${rack_fusion_admin_credentials_password}"}' -H "Content-Type: application/json" -H "Accept: application/json" "https://${bl_fusion_ip}/authn/rest/login-sessions" |cut -d'"' -f4
    ${sht_dict} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${rack_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${bl_fusion_ip}${shturi}"
    &{sht_dict} =     string to dictionary     ${sht_dict}
    ${sht_name}=    Get From Dictionary     ${sht_dict}    name
    [Return]    ${sht_name}
    Close Connection

Add server as managed
...
Check Profile if NonCompliant with Template
    ...
    ${ProfDetails} =    Execute Command    /usr/bin/curl -k -s -S -X GET -H "x-api-version: ${rack_api_version}" -H "Content-Type: application/json" -H "Accept: application/json" -H "auth:${output}" "https://${bl_fusion_ip}${svr_prof_uri}"
