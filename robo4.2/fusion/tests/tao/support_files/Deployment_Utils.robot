...
Import Certificate to OV
    ...
    [Arguments]    ${TAO_IP}    ${OV_IP}    ${OV_credentials}
    ...
    ${response} =  Fusion Api Get Server Certificate   TAO
...
Create Deployment Server Payload
    [Documentation]     Creates payload for Deployment server
    [Arguments]    ${osdeploymentserver}
    ${osds_body}=    Copy Dictionary    ${osdeploymentserver}
    ${resp} =    Fusion Api Get Ethernet Networks    param=?filter="'name'=='${TAO_Deploy_Nw}'"
    ${uri} =    Get From Dictionary    ${resp['members'][0]}    uri
    Set to Dictionary    ${osds_body['taoData']}    deploymentNetworkUri    ${uri}
    Set to Dictionary    ${osds_body['taoData']}    ip    ${TAO_IP}
    [Return]   ${osds_body}
...
First Time Setup with IPV6
    ...
    Deployment_Utils.Wait For Appliance To Become Pingable  ${APPLIANCE_IP}    5min    60sec
    Wait For Appliance To Be Ready    [${APPLIANCE_IP}]    20min    60sec
...
Update Appliance Data OV
    ...
    Set to dictionary   ${body['applianceNetworks'][0]}    app1Ipv4Addr  ${OV_IP}
    Run keyword if    '${OV_HOSTNAME}' != 'None'    Set to dictionary   ${body['applianceNetworks'][0]}    hostname  ${OV_HOSTNAME}
    Log    ${body}    console=True
    [return]    ${body}

Update Appliance Data TAO
    [Documentation]    New appliance IP, iptype and hostname will be updated at run time
    [Arguments]    ${body}
    ${resp} =   Fusion Api Get Appliance Interface Mac   eth0
    Set to dictionary   ${body['applianceNetworks'][1]}    macAddress  ${resp}
    Set to dictionary   ${body['applianceNetworks'][1]}    app1Ipv4Addr  ${TAO_IP}
    Run keyword if    '${TAO_HOSTNAME}' != 'None'    Set to dictionary   ${body['applianceNetworks'][1]}    hostname  ${TAO_HOSTNAME}
    Log    ${body}    console=True
    Connect To VI Server   ${VSPHERE_IP}    ${VSPHERE_USERNAME}    ${VSPHERE_PASSWORD}
    ${macaddress}=    Get VM Network Devices    ${FUSION_NAME}
    ${eth1}=    Get From List    ${macaddress}    1
    ${resp}=    Get From Dictionary   ${eth1}    mac_address
    Set to dictionary   ${body['applianceNetworks'][0]}    macAddress    ${resp}
    Set to dictionary   ${body['applianceNetworks'][0]}    app1Ipv4Addr    ${TAO_Deploy_IP}
    [return]    ${body}
