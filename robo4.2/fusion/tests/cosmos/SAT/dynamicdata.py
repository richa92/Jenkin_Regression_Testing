"""Dynamic Data file is used to create test data needed for creation/verification of resources"""

import re


class DynamicData(object):
    """Class for Dynamic Data functions"""
    ...
    def rename_enclosure(self, enclosures):
        """Generate Put data required to rename enclosure"""
        ret = []
    ...
    def expected_enclosure_data(self, enclosures):
        """Generate Get data to verify enclosure"""
        ret = []
        for enc in enclosures:
            ret.append({'name': enc['newName'], 'serialNumber': enc['serialNumber'], 'type': 'EnclosureV8'})
        return ret
    ...
    def expected_spp_data(self):
        """Make expected spp data dictionary"""
        ret = []
        ret.append({'type': 'firmware-baselines',
    ...
    def make_expected_lig_data(self, ligs):
        ...
        for lig in ligs:
            ...
            ret.append(
                {'name': lig['name'], 'type': 'logical-interconnect-groupV7', 'enclosureType': lig['enclosureType'],
    ...
    def create_bulk_network_set_data(self, ns):
        ...
        for nss in ns:
            for i in range(0, nss['count']):
                ...
                network_sets.append({'name': nss['namePrefix'] + str(i + 1), 'type': 'network-setV5',
    ...
    def make_expected_storage_system_with_pools_data(self, storagesystem):
        ...
        for exp_ssp in storagesystem:
            expected_storage_system_pools.append(
                {'type': 'StorageSystemV5', 'name': exp_ssp['name'], 'uri': 'SSYS:' + exp_ssp['name'],
                 'state': 'Managed',
    ...
    def expected_storage_system(self, storage_systems):
        ...
        for storage in storage_systems:
            stor.append({'type': 'StorageSystemV5',
    ...
    def expected_storage_volumes(self, storage_volumes):
        ...
        for exp_vols in storage_volumes:
            expected_vol.append(
                {'type': 'StorageVolumeV8', 'name': exp_vols['name'], 'status': 'OK', 'state': 'Managed'})
        return expected_vol
    ...
    def make_expected_enclousre_data(self, encls, spp):
        ...
        if spp is None:
            ...
            firmware_baseline_uri = '/rest/firmware-drivers/' + spp
        for encl in encls:
            ret.append({'type': 'EnclosureV8', 'name': encl['name'], 'state': 'Managed',
    ...
    def make_expected_server_profile_data(self, sps, spp, firmware):
        ...
        for sp in sps:
            ...
            if sp['bootMode']:
                ret.append(
                    {'name': sp['name'], 'type': 'ServerProfileV10', 'serverHardwareUri': "SH:" + sp['serverHardwareUri'],
