...
Status Poll
	...
	${server_hardware}=		Get From Dictionary		${server_profile_E1bay4}		serverHardwareUri
	${hardwareStatus}=	Get Server Hardware State	${server_hardware}
	${profileName}=		Get From Dictionary		${server_profile_E1bay4}		name
...
Create Server Profile
    ...
    Set Log Level	TRACE
    Power Off Server if On		${server_profile_E1bay4}
	
	${profileStatus}	${response}=  Run Keyword And Ignore Error		Add Server Profile  ${server_profile_E1bay4}
	#Run Keyword If		'${profileStatus}'=='FAIL'		Pause Execution    message=Create Server Profile E1bay4 failed. Press OK to continue
	
	${taskStatus}=		Run Keyword And Return Status	Wait For Task2	${response}   timeout=1200    interval=30
	#Run Keyword If		'${taskStatus}'=='False'		Pause Execution    message=Waiting for Task for Create Server Profile on E1bay4 Failed. Press OK to continue
	
	Run Keyword If		'${taskStatus}'=='False'  	Fail  Create server Profile E1bay4 failed 
	${verifyStatus}=	Run Keyword And Return Status	Verify Resource  ${expected_server_profile_E1bay4}
	#Run Keyword If		'${verifyStatus}'=='False'		Pause Execution    message=Verification for Create Server Profile on E1bay4 failed. Press OK to continue
	Run Keyword If		'${verifyStatus}'=='False'  Fail  Create server profile verification on E1bay4 failed


Add Server to Altair using ILO
	[Tags]    SPBay4
	[Documentation]	Add Servers to Altair using ILO
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		4
	
	${status}	${jobUri}=	Add Server using ILO		${server_profile_E1bay4}	${Servers[0].iloip}		${Servers[0].ilousername}		${Servers[0].ilopassword}
...
Install OS
	...
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		4
	${jobStatus}	${jobState}=	Run OSBP	${server_profile_E1bay4}	${Servers[0].OSBP}		${Servers[0].enclosure}		${Servers[0].bay}
...
Ping Server Open SSH Connection And Verify Disk
	...
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		4
	${ipv4addr}=	Get Server Management IP using Serial Number	${server_profile_E1bay4}	${Servers[0].enclosure}		${Servers[0].bay}
...
Edit Server Profile
    ...
    Set Log Level	TRACE
    Power Off Server if On		${server_profile_E1bay4}	${False}
	
	${profileStatus}	${response}=  Run Keyword And Ignore Error		Edit Server Profile  ${edit_server_profile_E1bay4}
	#Run Keyword If		'${profileStatus}'=='FAIL'		Pause Execution    message=Editing Server Profile E1bay4 failed. Press OK to continue
	
	${taskStatus}=		Run Keyword And Return Status	Wait For Task2	${response}   timeout=1200    interval=30
	#Run Keyword If		'${taskStatus}'=='False'		Pause Execution    message=Waiting for Task for Edit Server Profile on E1bay4 Failed. Press OK to continue
	Run Keyword If		'${taskStatus}'=='False'  	Fail  Edit server Profile E1bay4 failed 
	
	${verifyStatus}=	Run Keyword And Return Status	Verify Resource  ${expected_edit_server_profile_E1bay4}
	#Run Keyword If		'${verifyStatus}'=='False'		Pause Execution    message=Verification for Edit Server Profile on E1bay4 failed. Press OK to continue
	Run Keyword If		'${verifyStatus}'=='False'  Fail  Edit server profile verification on E1bay4 failed 
...
Erase Disk
	...
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		4
	${jobStatus}	${jobState}=	Run OSBP	${server_profile_E1bay4}	${Servers[0].EraseOSBP}		${Servers[0].enclosure}		${Servers[0].bay}
...
Delete Server
	...
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		4
	${response}=	Delete Server From Altair	${server_profile_E1bay4}	${Servers[0].enclosure}		${Servers[0].bay}
...
Remove Server Profile
    ...
    ${profileStatus}	${response}=  Run Keyword And Ignore Error		Remove Server Profile  ${edit_server_profile_E1bay4}
	#Run Keyword If		'${profileStatus}'=='FAIL'		Pause Execution    message=Removing Server Profile E1bay4 failed. Press OK to continue
	
	${taskStatus}=		Run Keyword And Return Status	Wait For Task2	${response}   timeout=1200    interval=30
	#Run Keyword If		'${taskStatus}'=='False'		Pause Execution    message=Waiting for Task for Remove Server Profile on E1bay4 Failed. Press OK to continue
	Run Keyword If		'${taskStatus}'=='False'  Fail  Remove server Profile E1bay4 failed
