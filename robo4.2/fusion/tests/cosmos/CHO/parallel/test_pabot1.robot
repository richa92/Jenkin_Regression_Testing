...
Status Poll
	...
	${profileName}=		Get From Dictionary		${server_profile_E1bay2}		name
...
Create Server Profile
    ...
    Set Log Level	TRACE
    Power Off Server if On		${server_profile_E1bay2}
	
	${profileStatus}	${response}=  Run Keyword And Ignore Error		Add Server Profile  ${server_profile_E1bay2}
	#Run Keyword If		'${profileStatus}'=='FAIL'		Pause Execution    message=Create Server Profile E1bay2 failed. Press OK to continue
	
	${taskStatus}=		Run Keyword And Return Status	Wait For Task2	${response}   timeout=1200    interval=30
	#Run Keyword If		'${taskStatus}'=='False'		Pause Execution    message=Waiting for Task for Create Server Profile on E1bay2 Failed. Press OK to continue
	Run Keyword If		'${taskStatus}'=='False'  	Fail  Create server Profile E1bay2 failed 
	
	${verifyStatus}=	Run Keyword And Return Status	Verify Resource  ${expected_server_profile_E1bay2}
	#Run Keyword If		'${verifyStatus}'=='False'		Pause Execution    message=Verification for Create Server Profile on E1bay2 failed. Press OK to continue
	Run Keyword If		'${verifyStatus}'=='False'  Fail  Create server profile verification on E1bay2 failed
...
Add Server to Altair using ILO
	...
	${status}	${jobUri}=	Add Server using ILO		${server_profile_E1bay2}	${Servers[0].iloip}		${Servers[0].ilousername}		${Servers[0].ilopassword}
...
Install OS
	...
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		2
	${jobStatus}	${jobState}=	Run OSBP	${server_profile_E1bay2}	${Servers[0].OSBP}		${Servers[0].enclosure}		${Servers[0].bay}
...
Edit Server Profile
    ...
    Set Log Level	TRACE
    Power Off Server if On		${server_profile_E1bay2}	${False}
	
	${profileStatus}	${response}=  Run Keyword And Ignore Error		Edit Server Profile  ${edit_server_profile_E1bay2}
	#Run Keyword If		'${profileStatus}'=='FAIL'		Pause Execution    message=Editing Server Profile E1bay2 failed. Press OK to continue
	
	${taskStatus}=		Run Keyword And Return Status	Wait For Task2	${response}   timeout=1200    interval=30
	#Run Keyword If		'${taskStatus}'=='False'		Pause Execution    message=Waiting for Task for Edit Server Profile on E1bay2 Failed. Press OK to continue
	Run Keyword If		'${taskStatus}'=='False'  	Fail  Edit server Profile E1bay2 failed 
	
	${verifyStatus}=	Run Keyword And Return Status	Verify Resource  ${expected_edit_server_profile_E1bay2}
	#Run Keyword If		'${verifyStatus}'=='False'		Pause Execution    message=Verification for Edit Server Profile on E1bay2 failed. Press OK to continue
	Run Keyword If		'${verifyStatus}'=='False'  Fail  Edit server profile verification on E1bay2 failed 
...
Delete Server
	...
	${Servers} =    Get Data By Property		${TestData.Servers}		ServerNo		2
	${response}=	Delete Server From Altair	${server_profile_E1bay2}	${Servers[0].enclosure}		${Servers[0].bay}
...
Remove Server Profile
    ...
    ${profileStatus}	${response}=  Run Keyword And Ignore Error		Remove Server Profile  ${edit_server_profile_E1bay2}
	...
	Run Keyword If		'${taskStatus}'=='False'  Fail  Remove server Profile E1bay2 failed 