...
Create Server Profiles with Storage Volumes
    ...
    Power off Servers in Profiles  ${sles15_electron16gb_fc_run2_profile}
    ${responses}=  Add Server Profiles from variable  ${sles15_electron16gb_fc_run2_profile}
    Run Keyword If  ${responses} is not ${null}  Wait For Task2  ${responses}
    ...  timeout=2000  interval=5
    Verify Resources for List  ${sles15_electron16gb_fc_run2_profile_expected}
...
Power On server Profile And Wait For Post State
    ...
    Power On Server Profile And Wait For POST State    ${sles15_electron16gb_fc_run2_profile}
...
Add Volume To Existing Profile, SLES Server Should Be Pinging, FCoE Disk Should Exist and Active
    ...
    Edit Server Profile And Power On    ${sles15_electron16gb_fc_run2_profile_add_volume}     ${sles15_electron16gb_fc_run2_profile_add_volume_expected}
    SLES Server Should Be Pinging And Volume Should Be Active    ${sles15_electron16gb_fc_run2_profile_add_volume}
...
Update Firmware From Server Profiles, SLES Server Should Be Pinging, FCoE Disk Should Exist and Active
    ...
    Edit Server Profile, Wait until firmware state reaches to desired state     ${sles15_electron16gb_fc_run2_profile_spp}    ${sles15_electron16gb_fc_run2_profile_spp_expected}
    SLES Server Should Be Pinging And Volume Should Be Active    ${sles15_electron16gb_fc_run2_profile_spp}
...
Enable NIC Teaming in SLES, SLES Server Should Be Pinging, FCoE Disk Should Exist and Active
    ...
    Create SLES Nic Bonding    ${sles15_electron16gb_fc_run2_profile}
    SLES Server Should Be Pinging And Volume Should Be Active    ${sles15_electron16gb_fc_run2_profile}
...
MAC address specified in profile and retrieved from OS should be same
    ...
    MAC address specified in profile and retrieved from SLES OS should be same   ${sles15_electron16gb_fc_run2_profile}
...
Remove Server Profile
    ...
    Power off Servers in Profiles  ${sles15_electron16gb_fc_run2_profile}
    ${responses}=  Remove Server Profiles from variable  ${sles15_electron16gb_fc_run2_profile}
    Run Keyword If  ${responses} is not ${null}  Wait For Task2  ${responses}
    ...  timeout=2000  interval=5
    Server Profile Should Not Exist  ${sles15_electron16gb_fc_run2_profile}

Restore System Default Settings for Servers
    [Documentation]    Restore Servers to System Default settings
    [Tags]    RESTORE-HW
    Check IloRestTool
    Power on Servers in Profiles   ${sles15_electron16gb_fc_run2_profile}
    Wait for Servers in Profiles to reach POST State  ${sles15_electron16gb_fc_run2_profile}  timeout=${timeout}  interval=${interval}
    Restore System Defaults Settings    ${sles15_electron16gb_fc_run2_profile[0]['serverHardwareUri']}    ${ilo_credentials['username']}    ${ilo_credentials['password']}
    Sleep    60 seconds
    Wait for Servers in Profiles to reach POST State  ${sles15_electron16gb_fc_run2_profile}  timeout=${timeout}  interval=${interval}