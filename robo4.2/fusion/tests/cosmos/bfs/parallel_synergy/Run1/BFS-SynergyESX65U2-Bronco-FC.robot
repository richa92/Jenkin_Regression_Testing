*** Settings ***
Documentation    ESX65U2_Bronco_FC_Run1
...
Create Server Profiles with Storage Volumes
    ...
    Power off Servers in Profiles  ${esx65u2_bronco_fc_run1_profile}
    ${responses}=  Add Server Profiles from variable  ${esx65u2_bronco_fc_run1_profile}
    Run Keyword If  ${responses} is not ${null}  Wait For Task2  ${responses}
    ...  timeout=2000  interval=5
    Verify Resources for List  ${esx65u2_bronco_fc_run1_profile_expected}
...
Power On server Profile And Wait For Post State
    ...
    Power On Server Profile And Wait For POST State    ${esx65u2_bronco_fc_run1_profile}
...
Add Volume To Existing Profile, ESXi Server Should Be Pinging, FCoE Disk Should Exist and Active
    ...
    Edit Server Profile And Power On    ${esx65u2_bronco_fc_run1_profile_add_volume}     ${esx65u2_bronco_fc_run1_profile_add_volume_expected}
    ESXi Server Should Be Pinging And Volume Should Be Active    ${esx65u2_bronco_fc_run1_profile_add_volume}
...
Update Firmware And Driver From Server Profiles, ESXi Server Should Be Pinging, FCoE Disk Should Exist and Active
    ...
    Edit Server Profile, Wait until firmware state reaches to desired state     ${esx65u2_bronco_fc_run1_profile_spp}    ${esx65u2_bronco_fc_run1_profile_spp_expected}
    ESXi Server Should Be Pinging And Volume Should Be Active    ${esx65u2_bronco_fc_run1_profile_spp}
...
Enable NIC Teaming in ESXi, ESXi Server Should Be Pinging, FCoE Disk Should Exist and Active
    ...
    Enable NIC Teaming in ESXi  ${esx65u2_bronco_fc_run1_profile}
    ESXi Server Should Be Pinging And Volume Should Be Active    ${esx65u2_bronco_fc_run1_profile}
...
MAC address specified in profile and retrieved from OS should be same
    ...
    MAC address specified in profile and retrieved from ESXi OS should be same   ${esx65u2_bronco_fc_run1_profile}
...
Remove Server Profile
    [Tags]   R-SP  ESX65U2_630_FCOE  RSP
    [Documentation]   Remove Server Profile and verify server profile doesn't exist
    Power off Servers in Profiles  ${esx65u2_bronco_fc_run1_profile}
    ${responses}=  Remove Server Profiles from variable  ${esx65u2_bronco_fc_run1_profile}
    Run Keyword If  ${responses} is not ${null}  Wait For Task2  ${responses}
    ...  timeout=2000  interval=5
    Server Profile Should Not Exist  ${esx65u2_bronco_fc_run1_profile}

Restore System Default Settings for Servers
    [Documentation]    Restore Servers to System Default settings
    [Tags]    RESTORE-HW    ESX65U2_630_FCOE  RSP
    Check IloRestTool
    Power on Servers in Profiles   ${esx65u2_bronco_fc_run1_profile}
    Wait for Servers in Profiles to reach POST State  ${esx65u2_bronco_fc_run1_profile}  timeout=${timeout}  interval=${interval}
    Restore System Defaults Settings    ${esx65u2_bronco_fc_run1_profile[0]['serverHardwareUri']}    ${ilo_credentials['username']}    ${ilo_credentials['password']}
    Sleep    60 seconds
    Wait for Servers in Profiles to reach POST State  ${esx65u2_bronco_fc_run1_profile}  timeout=${timeout}  interval=${interval}
