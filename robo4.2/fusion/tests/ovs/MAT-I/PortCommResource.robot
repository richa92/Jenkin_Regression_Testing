...
ILO4 Port Validation
    ...
    Log     Status:${status1}
    Run Keyword If   '${status1}' != 'True'    Log    Port of snmp is not enabled    WARN
    ...    ELSE IF    '${status1}' == 'True'    Set To Dictionary    ${ILO}    Port 161    Open
    ${status}=    Run Keyword and Return Status    Should Contain    ${ssl}    443
    Log     Status:${status}
    Run Keyword If   '${status}' != 'True'    Log    Port of ssl is not enabled    WARN
    ...    ELSE IF    '${status}' == 'True'    Set To Dictionary    ${ILO}    Port 443    Open
    Log Dictionary    ${ILO}
    [Return]    ${ILO}

ILO5 Port Validation
    [Documentation]    Checks port snmp and ssl
    ${ILO}=    Create Dictionary
    Open DevSSH Connection And Log In    ${ilo5_ip}    ${dev_cred['ilo5_user']}    ${dev_cred['ilo5_pass']}
    SSHLibrary.Write    ${ilocommand1}
    ${stdout1}=          SSHLibrary.Read until    ${ILO_PROMPT}
    SSHLibrary.Write    ${ilocommand2}
    ${stdout2}=          SSHLibrary.Read until    ${ILO_PROMPT}
    sleep    10
    ${ssl}=    Get Lines Containing String    ${stdout2}    oemHPE_sslport
    ${snmp}=    Get Lines Containing String    ${stdout2}    oemHPE_snmp_port
    Log    Print line ssl: ${ssl}    console=True
    Log    Print line snmp: ${snmp}    console=True
    ${status1}=    Run Keyword and Return Status    Should Contain    ${snmp}    161
    Log     Status:${status1}
...
IPDU Port Validation
    ...
    SSHLibrary.Execute Command     nc -vzu ${ipdu_ip} 161 &> output.txt
    ${snmp}=    SSHlibrary.Execute Command    cat output.txt
    Log    ${snmp}    console=True
    ${status1}=    Run Keyword and Return Status    Should Contain    ${snmp}    UDP packet sent successfully
    Run Keyword If   '${status1}' != 'True'    Log    SNMP port is not enabled for IPDU    WARN
...
3PAR Port Validation
    ...
    SSHLibrary.Execute Command     nc -vzu ${stor_ip} 161 &> 3par.txt
    ${snmp}=    SSHLibrary.Execute Command    cat 3par.txt
    Log    ${snmp}    console=true
    SSHLibrary.Execute Command     nc -vzu ${stor_ip} 8080 &>3par1.txt
    ${https}=    SSHLibrary.Execute Command    cat 3par1.txt
    Log    ${https}    console=true
    ${status1}=    Run Keyword and Return Status    Should Contain    ${snmp}    UDP packet sent successfully
    Run Keyword If   '${status1}' != 'True'    Log    SNMP port is not enabled for 3PAR    WARN
    ...    ELSE IF    '${status1}' == 'True'    Set To Dictionary    ${3PAR}    Port 161    Open
    ${status}=    Run Keyword and Return Status    Should Contain    ${https}    UDP packet sent successfully
    Run Keyword If   '${status}' != 'True'    Log    HTTPS port is not enabled for 3PAR    WARN
    ...    ELSE IF    '${status}' == 'True'    Set To Dictionary    ${3PAR}    Port 8080    Open
    Log Dictionary    ${3PAR}
    [Return]    ${3PAR}

Check Port State
    [Documentation]    Check Port State
    [Arguments]    ${result}
    ...
    ...    ELSE IF   '${snmp}' == '${PASS_status}'    Log    PORT 161 CHECK PASSED    console=true
    ...
    ...    ELSE IF     '${http}' == '${PASS_status}'   Log    PORT 443 CHECK PASSED    console=true
    ...
    ...    ELSE IF     '${ssh}' == '${PASS_status}'    Log    PORT 22 CHECK PASSED    console=true

Parse Key Value Output
    [Documentation]    Parse Key value Output
    [Arguments]    ${result}
...
Port Communication Check for OA
    ...
    ...    ELSE IF    '${Status}' == 'True'    Log    ********Port Communication check for OA PASSED*******    console=true
...
Port Communication Check for ILO4
    ...
    ...    ELSE IF    '${Status}' == 'True'    Log    ********Port Communication check for ILO4 PASSED********    console=true

Port Communication Check for ILO5
    [Documentation]     Port Communication check for ILO5
    ${ILO}=    ILO5 Port Validation
    Log    Validate:${ILO}    console=True
    Open DevSSH Connection And Log In    ${SSH_HOST}    ${HOST_USERNAME}    ${HOST_PASSWORD}
    ${ILO5}=  SSHLibrary.Execute Command    cd /ci/support/ovsupportability/scripts/ && ./check_port_communication.py -ip ${ilo5_ip} -devtype iLo
    log    Command output:Port communication for ILO5    console=true
    log    ${ILO5}    console=true
    ${ILO5}=    Parse Key Value Output    ${ILO5}
    Log    Parse:${ILO5}    console=True
    ${Status}=    Run Keyword and Return Status    Dictionaries Should Be Equal    ${ILO5}    ${ILO}
    Log    status:${Status}    console=True
    Run Keyword If    '${Status}' != 'True'    Fail    ********Port Communication check for ILO5 FAILED********
    ...    ELSE IF    '${Status}' == 'True'    Log    ********Port Communication check for ILO5 PASSED********    console=true
...
Port Communication Check for ILO3
    ...
    ...    ELSE IF    '${Status}' == 'True'    Log    ********Port Communication check for ILO3 PASSED********    console=true
...
Port Communication Check for 3PAR
    ...
    ...    ELSE IF    '${Status}' == 'True'    Log    ********Port Communication check for 3PAR PASSED********    console=true
...
Port Communication Check for IPDU
    ...
    ...    ELSE IF    '${Status}' == 'True'    Log    ********Port Communication check for IPDU PASSED********    console=true

 Port Communication Help Command
    [Documentation]    Port communication check for Help Command
     Open DevSSH Connection And Log In    ${SSH_HOST}    ${HOST_USERNAME}    ${HOST_PASSWORD}
