...
class DynamicData(object):
    ...
    def get_hafnium_pkg_name(self, url):
        pkg = re.search(r'hpe_.*.pkg', url)
        return pkg.group()

    def get_hafnium_version(self, url):
        temp = re.search(r'fv.*./', url)
        fw = temp.group()[4:-1]
        version = fw.replace('-', '.')
        return version
    ...
    def make_expected_server_profile_data(self, sps, firmware):
        ...
        for sp in sps:
            ret.append({'name': sp['name'], 'type': 'ServerProfileV8', 'serverHardwareUri': sp['serverHardwareUri'],
                        ...
                        # 'firmware': {'firmwareInstallType': firmware['firmwareInstallType'], 'manageFirmware': firmware['manageFirmware']},
        ...
        for sp in sps:
            for spt in spts:
                ...
                if sp['serverProfileTemplateUri'] == serverProfileTemplateUri:
                    ...
                    boot = spt['boot']
                    bios = spt['bios']
                    localStorage = spt['localStorage']
                    sanStorage = spt['sanStorage']
            ret.append({'name': sp['name'], 'type': 'ServerProfileV8', 'serverHardwareUri': 'SH:' + sp['serverHardwareUri'],
                        'connections': connections, 'status': 'OK', 'state': 'Normal', 'bootMode': bootMode, 'boot': boot,
                        'bios': bios, 'localStorage': localStorage, 'sanStorage': sanStorage})
        return ret

    def make_expected_logical_enclosure_data(self, les):
        ret = []
        for le in les:
            ret.append({'type': 'LogicalEnclosureV4', 'name': le['name'], 'status': 'OK',
