...
${VERIFY}           True
${giSleep}          450
${fusion_api_resource}      E:/Shares/testrepo/prod/fusion/Resources/api/fusion_api_resource.txt
...
Trigger on 1 node
    [Documentation]    Trigger reimage on 1 node
    ${ip} =     Get From List    ${ov_appliances}    0
    ${command} =    Set Variable    /ci/etc/usb-reimage/developer_usb_reimage.sh -R
    ov reimage    ${ip}    ${command}

ov reimage
    [Documentation]    Run OV reimage command and wait unitll it completes
