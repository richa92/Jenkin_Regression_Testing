...
Create I3S Server Profile
    ...
    ${sht} =    Get Server Hardware Type By Server Hardware    ${sp['serverHardwareUri']}
    Set To Dictionary    ${sp}    serverHardwareTypeUri=${sht}
...
Edit I3S Server Profile
    ...
    ${sht} =    Get Server Hardware Type By Server Hardware    ${sp3['serverHardwareUri']}
    Set To Dictionary    ${sp3}    serverHardwareTypeUri=${sht}
...
Remove SP By Name
    [Documentation]  Remove Server Profile By Name
    [Arguments]    ${sp}

    ${sp_uri} =    Get Server Profile URI    ${sp}
    Return From Keyword If  '${sp_uri}'== '/rest/server_profile_uri_${sp}_not_found'
    ${resp_delete_sp} =   Fusion Api Delete Server Profile    uri=${sp_uri}
    Wait For Task2    ${resp_delete_sp}    timeout=300    interval=5
