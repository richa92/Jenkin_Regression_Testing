...
Get Server Hardware Type By Server Hardware
    [Documentation]   Get the DTO for a Server Hardware and return the serverHardwareType
    [Arguments]     ${name}

    ${server} =    Get Resource    ${name}
    ${status}    ${shtUri} =    Run Keyword And Ignore Error    Get From Dictionary    ${server}  serverHardwareTypeUri
    # if Server Hardware not found, then return /BadUri
    Return From Keyword If    '${status}'=='FAIL'    /BadUri
    ${shtName} =    Get Resource by URI    ${shtUri}
    [Return]    SHT:${shtName['name']}
