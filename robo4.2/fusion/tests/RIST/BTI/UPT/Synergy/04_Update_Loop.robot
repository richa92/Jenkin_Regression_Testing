...
*** Test Cases ***
Synergy UPT Update Loop Log Check Update Hops Files
...
Synergy UPT Update Loop Build Updatebin Files
...
Synergy UPT Update Loop Add Preupdate Resources
	...
	Log  \nAdd preupdate profiles  console=True
    ...
    Log  \nGet preupdate interconnects ports status
    Get All Interconnects Ports Status
    Log  \nGet preupdate profiles alerts  console=True
...
Synergy UPT Update Loop Update the Appliance
    [Documentation]  Update the appliance through the hops
...
Syerngy UPT Update Loop Get Profiles Critical Alerts
...
Syerngy UPT Update Loop Get Profiles Warning Alerts
...
Syerngy UPT Update Loop Set Do Not Delete
...
Update the Appliance
    ...
    Wait Until Keyword Succeeds  ${PRIMARY_APPLIANCE_UPDATE_TIMEOUT}  1 m  Appliance State Should Match  ((?i)OK)
    Log  \nWaiting for OneView state to be UPGRADE  console=True
...
Appliance Active Node State Should be OK
    [Documentation]  Check Appliance Active Node State
    ${resp} =  fusion api get resource  /rest/appliance/ha-nodes
    :FOR  ${member}  IN  @{resp['members']}
    \   continue for loop if  '${member['role']}'=='Standby'
    \   Log  \nAppliance ${member['role']} node state: ${member['state']}  console=True
    \   Should be equal  ${member['state']}  OK  msg=${member['role']} node state

Appliance Standby Node State Should be OK
    [Documentation]  Check Appliance Active Node State
    ${resp} =  fusion api get resource  /rest/appliance/ha-nodes
    :FOR  ${member}  IN  @{resp['members']}
    \   continue for loop if  '${member['role']}'=='Active'
    \   Log  \nAppliance ${member['role']} node state: ${member['state']}  console=True
    \   Should be equal  ${member['state']}  OK  msg=${member['role']} node state

Wait for Standby Node State To be OK
    [Documentation]  wait for standby node state to be ok
    [Arguments]  ${timeout}=40min  ${interval}=1m
    Wait Until Keyword Succeeds  ${timeout}  ${interval}  Appliance Standby Node State Should be OK
...
Postupdate Checks
    ...
    Wait for task2  ${resp}  timeout=${STANDBY_APPLIANCE_UPDATE_TIMEOUT}  interval=10
    Log  \nCheck appliance software version
    ${build}=  Get Updatebin Build  ${updatebin_files["${hop}"]}
    Check Appliance Software Version  ${build}
	Log  \nCreate Network logs
	Create Network Logs  ${build}
    Log  \nWait for appliance standby node to be on-line...
    Wait for Standby Node State To be OK  timeout=30m  interval=1m
