...
Create SP on Gen 9 server with 2 connections - Boot settings - Manage boot mode - Select boot mode as "UEFI" - with OS volume
    ${rc}                               Fusion UI Create Server Profile                               @{TestData.F1407UI071.Create}
    ...
    ${rc}                               Fusion UI Verify Server Profile San Storage Info              @{TestData.F1407UI071.Create}
    Should Be True                      ${rc}   msg=Failed to verify Server Profile San Storage info

#    ${rc}                               Fusion UI Power On Server Profile                             @{TestData.F1407UI071.Create}
#    Should Be True                      ${rc}   msg=Failed to power on server profile
#
#    Log To Console And Logfile          \nPOST check to make sure POST is finished
#    Fusion Api Login Appliance          ${APPLIANCE_IP}        ${admin_credentials}
#    Wait for Server to reach POST State		 ${TestData.F1407UI071.Create[0].server}       ${post_state}