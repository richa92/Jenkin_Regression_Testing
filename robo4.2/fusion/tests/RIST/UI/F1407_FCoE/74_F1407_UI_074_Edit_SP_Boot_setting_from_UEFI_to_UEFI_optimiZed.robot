...
Edit Server Profile Boot Setting from UEFI to UEFI_OptimiZed
    ${rc}                               Fusion UI Edit Server Profile                                 @{TestData.F1407UI074.Edit}
    Should Be True                      ${rc}   msg=Failed to Edit Server Profile Boot Setting from UEFI to UEFI_OptimiZed

    ${rc}                               Fusion UI Verify Server Profile San Storage Info              @{TestData.F1407UI074.Verify}
    Should Be True                      ${rc}   msg=Failed to verify Server Profile San Storage info

#    ${rc}                               Fusion UI Power On Server Profile                   @{TestData.F1407UI074.Create}
#    Should Be True                      ${rc}   msg=Failed to power on server profile
#
#    Log To Console And Logfile          \nPOST check to make sure POST is finished
#    Fusion Api Login Appliance          ${APPLIANCE_IP}        ${admin_credentials}
#    Wait for Server to reach POST State		 ${TestData.F1407UI074.Create[0].server}       ${post_state}