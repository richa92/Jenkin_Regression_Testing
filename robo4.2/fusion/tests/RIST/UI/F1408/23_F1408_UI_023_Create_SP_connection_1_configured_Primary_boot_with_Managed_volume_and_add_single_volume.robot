...
Create SP with connection 1 is configured to Primary boot - with Managed volume - Add single volume
    ${rc}                               Fusion UI Create Server Profile                     @{TestData.F1408UI023.Create}
    ...
    ${rc}                               Fusion UI Verify Server Profile SAN Storage Info    @{TestData.F1408UI023.Create}
    Should Be True                      ${rc}   msg=Failed to verify the private volume in the server profile

    ${rc}                               Fusion UI Verify Server Profile Connections Info    @{TestData.F1408UI023.Create}
