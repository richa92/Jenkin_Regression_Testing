...
Create SP with LUN manually addigned
    ${rc}                               Fusion UI Create Server Profile                     @{TestData.F1408UI024.Create}
    Should Be True                      ${rc}   msg=Failed to create SP with LUN number manually assigned

    ${rc}                               Fusion UI Verify Server Profile SAN Storage Info    @{TestData.F1408UI024.Create}
    Should Be True                      ${rc}   msg=Failed to verify the private volume in the server profile

    ${rc}                               Fusion UI Verify Server Profile Connections Info    @{TestData.F1408UI024.Create}
