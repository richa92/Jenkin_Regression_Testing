...
Edit the server profile to Modify the storage paths under SAN storage --> add volume section Add one more path
    ${rc} =    Fusion Ui Create Server Profile     @{TestData.F221P059.Create}
    Should Be True    ${rc}   msg=Failed to create the server profile
    ${rc} =    Fusion Ui Edit Server Profile     @{TestData.F221P059.Edit}
    ...
    ${rc} =    Fusion UI Verify Server Profile SAN Storage Info    @{TestData.F221P059.Edit}
    Should Be True    ${rc}   msg=Failed to verify the private volume in the server profile
    ${rc} =    Fusion UI Verify Server Profile Connections Info    @{TestData.F221P059.Edit}
