...
Edit SP with Port parameter from Auto to None
    ${rc}                               Fusion UI Create Server Profile                             @{TestData.F1407UI100.Create}
    Should Be True                      ${rc}   msg=Failed to Create Server Profile with Port parameter - auto

    ${expectedErrorMessage}=  catenate   ${nonConnectionIsConfigurededErrMsgPart1}  ${TestData.F1407UI100.Volname}  ${nonConnectionIsConfigurededErrMsgPart3}
    ${expectedErrorMessage}=  catenate  SEPARATOR=    ${unableToEditProfileErrMessage}    ${expectedErrorMessage}
    Run Keyword and Expect Error   ${expectedErrorMessage}  Fusion UI Edit Server Profile             @{TestData.F1407UI100.Edit}