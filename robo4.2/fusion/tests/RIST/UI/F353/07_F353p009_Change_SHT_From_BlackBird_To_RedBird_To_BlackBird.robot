...
*** Test Cases ***
F353p009_Change_SHT_From_BlackBird_To_RedBird_To_BlackBird
  ${Result} =      Fusion UI Create Server Profile  @{TestData.F353P003.Create}
    ...
    should be true   ${Result}    msg=Failed to verify Server Profile from General Info
    ${Result} =      Fusion UI Edit Server Profile   @{TestData.F353P003.Edit_blackbird}
    should be true   ${Result}   msg=Failed to edit Server Profiles
    ${Result} =      Fusion UI Verify Server Profile Advanced Info   @{TestData.F353P003.Verify_blackbird}
    should be true   ${Result}    msg=Failed to verify Server Profile Advanced Info
    ${Result} =      Fusion UI Verify Server Profile General Info    @{TestData.F353P003.Verify_blackbird}
    should be true   ${Result}    msg=Failed to verify Server Profile from General Info
