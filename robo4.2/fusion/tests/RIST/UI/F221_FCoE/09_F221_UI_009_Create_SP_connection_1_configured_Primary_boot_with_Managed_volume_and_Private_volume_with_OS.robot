...
Validation of server profile creation when connection 1 is configured to Primary boot - with Managed volume Private Volume with OS.
    ${rc}                               Fusion UI Create Server Profile                     @{TestData.F221UI009.Create}
    ...
    ${rc}                               Fusion UI Verify Server Profile SAN Storage Info    @{TestData.F221UI009.Create}
    Should Be True                      ${rc}   msg=Failed to verify the private volume in the server profile

    ${rc}                               Fusion UI Verify Server Profile Connections Info    @{TestData.F221UI009.Create}
