...
Create SP with LUN auto addigned and verify the same
    ${rc}                               Fusion UI Create Server Profile                     @{TestData.F221UI025.Create}
    Should Be True                      ${rc}   msg=Failed to create SP with LUN number auto assigned

    ${rc}                               Fusion UI Verify Server Profile SAN Storage Info    @{TestData.F221UI025.Create}
    Should Be True                      ${rc}   msg=Failed to verify the private volume in the server profile

    ${rc}                               Fusion UI Verify Server Profile Connections Info    @{TestData.F221UI025.Create}
