...
*** Test Cases ***
As an Administrator I want to create SP with physical MAC, WWN,SN
    ${Result} =      Fusion UI Create Server Profile  @{TestData.F289P012}
    should be true   ${Result}   msg=Failed to create Server Profile

    ${Result} =      Fusion UI Verify Server Profile Advanced Info   @{TestData.F289P012}
    should be true   ${Result}    msg=Failed to verify Server Profile Advanced Info

    ${Result} =      Fusion UI Verify Server Profile General Info    @{TestData.F289P012}
