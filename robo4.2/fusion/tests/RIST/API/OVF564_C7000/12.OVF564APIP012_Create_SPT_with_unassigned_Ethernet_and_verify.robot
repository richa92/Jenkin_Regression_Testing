*** Settings ***
Documentation        OVF564 Create SPT with Ethernet function type and unassigned network
...
Create SP with Ethernet function type and unassigned network
    ...
    ${resps} =    Add Server Profile Templates from variable    ${createSPTP012}

    :FOR    ${resp}    IN    @{resps}
    \    ${task} =    Wait For Task2     ${resp}    20min    5    None Expected    Completed

    ${rc} =  Validate Server Profile Templates Connections Network Applied  ${createSPTP012}
