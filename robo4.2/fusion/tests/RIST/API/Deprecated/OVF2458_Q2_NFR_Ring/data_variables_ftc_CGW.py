...
users = [
    {'userName': 'lori', 'password': 'hpvse123', 'fullName': 'Lori Winter', 'permissions': [{'roleName': 'Infrastructure administrator'}],
     'emailAddress': 'lori.winter@hpe.com', 'officePhone': '970-555-0003', 'mobilePhone': '970-500-0003',
     'type': 'UserAndPermissions'},
    {'userName': 'russell', 'password': 'hpvse123', 'fullName': 'Russell Briggs', 'permissions': [{'roleName': 'Infrastructure administrator'}],
    ...
    {'userName': 'fvtftc', 'password': 'fvtftchpvse123', 'fullName': 'Ron Soto and Team',
     ...
     'emailAddress': 'ron.soto@hpe.com', 'officePhone': '970-555-0003', 'mobilePhone': '970-500-0003',
     'type': 'UserAndPermissions'},
    {'userName': 'Serveradmin', 'password': 'Serveradmin', 'fullName': 'Serveradmin', 'permissions': [{'roleName': 'Server administrator'}],
    ...
    {'userName': 'Networkadmin', 'password': 'Networkadmin', 'fullName': 'Networkadmin',
     'permissions': [{'roleName': 'Network administrator'}], 'emailAddress': 'nat@hp.com', 'officePhone': '970-555-0003',
     'mobilePhone': '970-500-0003', 'type': 'UserAndPermissions'},
    {'userName': 'Backupadmin', 'password': 'Backupadmin', 'fullName': 'Backupadmin', 'permissions': [{'roleName': 'Backup administrator'}],
     'emailAddress': 'backup@hp.com', 'officePhone': '970-555-0003', 'mobilePhone': '970-500-0003',
     'type': 'UserAndPermissions'},
    {'userName': 'Noprivledge', 'password': 'Noprivledge', 'fullName': 'Noprivledge', 'permissions': [{'roleName': 'Read only'}],
...
enc_groups = {EG1: {'name': EG1,
                    ...
                    'interconnectBayMappings':
                        [{'interconnectBay': 1, 'logicalInterconnectGroupUri': 'SASLIG:%s' % LIG1},
                         {'interconnectBay': 2,
                          'logicalInterconnectGroupUri': 'LIG:%s' % LIG2},
                         {'interconnectBay': 3,
                          'logicalInterconnectGroupUri': 'LIG:%s' % LIG3},
                         {'interconnectBay': 4,
                          'logicalInterconnectGroupUri': 'SASLIG:%s' % LIG1},
                         {'interconnectBay': 5,
                          'logicalInterconnectGroupUri': 'LIG:%s' % LIG2},
...
sp2bm1 = {
    '%s_BAY_%i_480_1' %
    (e, i): '%s, bay %i' %
    (e, i) for i in bays_480 for e in ENCS}
sp2bm2 = {
    '%s_BAY_%i_660_1' %
    (e, i): '%s, bay %i' %
    (e, i) for i in bays_660 for e in ENCS}
...
volnames_private_r1 = [
    'FC_r1_Private_%s_Bay%s' %
    (e, b) for e in enc_short for b in bays_480]
volnames_private_r5 = [
    'FC_r5_Private_%s_Bay%s' %
    (e, b) for e in enc_short for b in bays_660]
...
# storage system targets.  Used for Validation not assigned by OneView at
# this time
storage_targets = [
    "20:12:00:02:AC:00:AF:0A",
...
attach_template = {
    ...
    # Existing volumes have these next three attributes.  For OV created volume we will replace these with
    # new_volume_template{} defined below.
    "volume": None,
    "volumeUri": "",
...
new_volume_template = {"volume": {
    "properties": {
        "name": "",
                ...
                "storagePool": "SPOOL:" + STORESERV1_POOL2,

    },
    "isPermanent": False,
    "templateUri": "ROOT:" + STORESERV1_POOL2,
}}
...
for e in ENCS:
    ...
    for b in bays:
        ...
        this_create.pop('volume')
        this_create.pop('volumeUri')
        this_create.pop('volumeStorageSystemUri')

        this_volume_create = deepcopy(new_volume_template)
        this_volume_create['volume']['properties']['name'] = ov_create_name
        this_volume_create['volume']['properties']['size'] = one_gb * b

        # attach new_volume_template to this_create{}
        this_create['volume'] = this_volume_create['volume']
        this_create['id'] = id
        id += 1

        # now append to list of volume attachments for this profile
        ...
        # bays 3 and 4 are windows.  The rest are ESXi
        if b <= 4:
            os_type = os_types['WIN']
        else:
            os_type = os_types['ESXi']
