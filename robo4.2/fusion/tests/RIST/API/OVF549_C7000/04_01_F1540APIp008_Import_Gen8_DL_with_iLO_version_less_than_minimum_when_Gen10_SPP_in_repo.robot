*** Settings ***
Documentation        Import Gen8 DL with iLO version less than minimum when Gen10 SPP in repo(Firmware should be updated)
...
*** Test Cases ***
Import Gen8 DL with iLO version less than minimum when Gen10 SPP in repo(Firmware should be updated)
    Log    \nTest to downgrade DL server ilo and add it to appliance    console=true
    Reset iLO and Update Server ILO Firmware      ${G8DLServer}
    Add Server Hardware Async  ${G8DLAdd}    ${TRUE}
    :FOR    ${sh}  IN  @{G8DLAdd}
