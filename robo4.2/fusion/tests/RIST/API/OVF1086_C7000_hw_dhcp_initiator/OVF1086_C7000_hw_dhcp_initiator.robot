*** Settings ***
Documentation                   OVF1086/OVS3803/F1321 Simplified iSCSI Boot Server Profile Templates for C7000
...
Create Profile from Template using Template Defined Volumes
    ...
    [Tags]    OVF1086-c7k-d-6  Performance  server_profiles-condition-iscsi_managed_volume
...
Edit Profile from Template to use Existing Volume
    ...
    [Tags]    OVF1086-c7k-d-8  Performance  server_profiles-condition-iscsi_managed_volume
    Power Off Servers in Profiles  ${edit_sp_from_spt}
...
Edit Server Profiles to be Non-compliant
    ...
    [Tags]    OVF1086-c7k-d-13
    Power Off Servers in Profiles  ${edit_compliance1_sp_from_spt}
...
Gen10 DHCP tests
    [Tags]    OVF1086-c7k-d-17
    ${resp_list} =  Add Server Profile Templates from variable  ${create_gen10_templates}
    Wait For Task2  ${resp_list}  timeout=60  interval=10
    ${resp_list} =  Edit Server Profile Templates from variable  ${edit_gen10_templates}
    Wait For Task2  ${resp_list}  timeout=60  interval=10
    Power Off Servers in Profiles  ${create_gen10_sp_from_spt}
    ${resp_list}=  Add Server Profiles from variable  ${create_gen10_sp_from_spt}
    Wait For Task2  ${resp_list}  timeout=1500  interval=10
    :FOR  ${profile}  IN  @{verify_create_gen10_sp_from_spt}
    \   Verify Server Profile  ${profile}
    \   ${task}=  Patch Server Profile  ${profile}  # default behavior is to request SPT compliance
    \   Wait For Task2  ${task}  timeout=600  interval=10
    Power Off Servers in Profiles  ${create_gen10_sp_from_spt}
    ${resp_list}=  Remove Server Profiles from variable	 ${create_gen10_sp_from_spt}  force=${True}
    Get Task and Wait for Completion  ${resp_list}  3600
    :FOR   ${spt_name}  IN  @{delete_gen10_profile_templates}
    \      Fusion Api Delete Server Profile Template  ${spt_name}
    ${resplist} =  Remove Storage Volumes Async  ${gen10_new_permanent_volumes}  param=?suppressDeviceUpdates=false
