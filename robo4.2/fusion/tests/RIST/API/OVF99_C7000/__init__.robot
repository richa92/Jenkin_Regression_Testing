...
${Team_Name}                    SHQA
${Ring}                         WPST32
${UIDataFile}                   OVF99_C7000/Regression_data.xml  # Data File Location
${FTS}                          false
${Add_Storage}                  false
${Add_Enclosure}                true
...
Precondition Setup
    ...
    ${task} =   Wait For Task2           ${resp}     800    5
    ...
    Log     ia_scope scope uri is ${scope}    console=true
    ${scopes} =  Create List    ${scope}
    Log     create list scopes is ${scopes}    console=true
    ...
    ${sh1_uri} =    Get Server Hardware URI    ${SH1}
    ${sh2_uri} =    Get Server Hardware URI    ${SH2}
    ${sh3_uri} =    Get Server Hardware URI    ${SH3}
    ${scopes} =  Create List
    :FOR    ${scope_name}    IN    @{ia_user_scopes}
    \    ${scope} =    Get Scope URI By Name    ${scope_name}
    ...
    Wait For Task2    ${resp}
    ${resp} =    Edit Resource Scope    ${sh1_uri}    ${scopes}
    Wait For Task2    ${resp}
    ${resp} =    Edit Resource Scope    ${sh2_uri}    ${scopes}
    Wait For Task2    ${resp}
    ${resp} =    Edit Resource Scope    ${sh3_uri}    ${scopes}
    Wait For Task2    ${resp}
