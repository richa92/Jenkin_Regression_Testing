...
*** Test Cases ***
IA can not create SP from SPT page and using SPP which is not in its scope to do firmware offline update
    set log level    TRACE
    ...
    ${firmware} =  Generate Firmware Body  ${Gen9Firmware}   ${offline_type}
    ...
    log    server profile template uri is ${server_profile_template_uri}    console=true

    ${createBLProfileFromSPT} =    Set To Dictionary    ${createBLProfileFromSPT}    serverProfileTemplateUri    ${server_profile_template_uri}
    ${resp} =    Add Server Profile From SPT With Firmware and Scopes   ${createBLProfileFromSPT}    ${ia_part_scopes}  ${Gen10Firmware}   ${offline_type}    ${sp_param}
