...
*** Test Cases ***
IA can create SP and using SPP which is in selected scope to do firmware offline update
    set log level    TRACE
    ...
    Set To Dictionary    ${createBLProfile}    firmware    ${firmware}
    ${resp} =    Add Server Profile With Spcopes   ${createBLProfile}    ${ia_user_scopes}
    Wait For Task2    ${resp}   3000

    Log    \n validate profile firmware applied    console=true
    Validate Profile Firmware Applied    ${createBLProfile}
    Validate Firmware Installed    ${createBLProfile}    ${Gen10Firmware}
