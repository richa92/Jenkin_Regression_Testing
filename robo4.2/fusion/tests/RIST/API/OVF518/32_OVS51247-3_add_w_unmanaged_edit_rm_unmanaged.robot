*** Settings ***
Documentation   Positive tests: Edit unassigned SP, remove unmanaged connections.
...
TS0 Create the Profile
    [Tags]   TS0
    ${resp} = 	Add Server Profile	${OVS51247_add_unassigned_T3}
	Wait For Task2  ${resp}     timeout=2400    interval=2

TS1 Edit the Profile
    [Tags]    TS1
    ${new_name} =    Set Variable    OVF518_OVS51247_unassigned_rm_unman_conn
    ${resp} =  Edit Server Profile  ${OVS51247_edit_unassigned_T3}    newname=${new_name}
    log    ${resp}    console=true
	Wait For Task2  ${resp}  timeout=2400  interval=2
	Set To Dictionary    ${OVS51247_edit_unassigned_T3}    name    ${new_name}
    Verify Server Profile  ${OVS51247_edit_unassigned_T3}
