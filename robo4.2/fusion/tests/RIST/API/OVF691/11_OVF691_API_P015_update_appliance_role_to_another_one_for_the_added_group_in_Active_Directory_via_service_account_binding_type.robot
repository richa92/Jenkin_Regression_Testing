...
...                  OVF691_API_P015_update appliance role to another one for the added group in Active Directory via service account binding type
...
*** Test Cases ***
OVF691_API_P015_update appliance role to another one for the added group in Active Directory via service account binding type
    [Documentation]  OVF691_API_P015_update appliance role to another one for the added group in Active Directory via service account binding type
    ...
    ${body}=  Generate Post Grouptorolemapping Payload     ${RN_IA}
    ${resp}=  fusion api assign roles to directory group   ${body}
    Wait For Task2       ${resp}     50    5

    Log    \n-Verify group info...    console=true
    ${result}=  Verify Group Info    ${RN_IA}

    Log    \n-Update group to role mapping...    console=true
    ${body}=  Generate Put Grouptorolemapping Payload
    ${resp}=  fusion api update group role assignment     ${body}
    Wait For Task2       ${resp}     50    5
