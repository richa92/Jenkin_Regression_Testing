*** Settings ***
Documentation        Cannot connect to rabbitmq server with incorrect cachain for extrernal ca signed rabbtimq client cert
...
*** Test Cases ***
Connect To Rabbitmq server with external ca signed rabbitmq client cert with incorrect external cachain
    [Documentation]    Cannot connect to rabbitmq server with incorrect cachain for extrernal ca signed rabbtimq client cert
    Fusion Api Login Appliance    ${APPLIANCE_IP}    ${admin_cred}

	Log    \n-Add external root CA certificate that signed rabbitmq client cert to OV    console=yes
    Set to Dictionary    ${ca_cert_body['members'][0]['certificateDetails']}    base64Data    ${client_files[2]}
    Set to Dictionary    ${ca_cert_body['members'][0]['certificateDetails']}    aliasName    external_rootca
    Log    ${ca_cert_body}
