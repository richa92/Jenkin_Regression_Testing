*** Settings ***
Documentation        Can connect to rabbitmq server after backup restore when appliance cert is self signed one
...
*** Test Cases ***
Connect To Rabbitmq server after backup restore when appliance cert is self signed one
    [Documentation]    When the appliance cert is the self signed one, it can connect to rabbitmq server after backup restore by using correct
    ...
    Log    \n-Get client cert files again    console=yes
    ${resp} =  Get Client Files For Internal Ca Signed Rabbitmq Client Cert    ${files_for_ca_signed_appliance_cert[3]}
    Should Be True    ${resp}    msg=Fail to get client files for internal ca signed rabbitmq client cert
