*** Settings ***
Documentation        FC supports SBAC Update an FC network scope which is the active scope to delete an active scope
...
*** Test Cases ***
FC supports SBAC Update an FC network scope which is the active scope to delete an active scope
    [Documentation]  OVF1592 API n011 FC supports SBAC Update an FC network scope which is the active scope to delete an active scope
    ...
    Log    Deleting Test from fc2   console=True
    ${resp}=   Patch Resources Scopes  FC:${fc_networks[1]['name']}   ${body}
