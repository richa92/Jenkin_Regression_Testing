...
*** Test Cases ***
[SPA] - Update a server profile and select a existing volume which is in the same active scope with SP
    [Documentation]  OVF1592 API p191 [Server Profile Administrator] Update a server profile and select a existing volume which is in the same active scope with SP
