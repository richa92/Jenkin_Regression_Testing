...
*** Test Cases ***
[SPA] - Delete a server profile in active scope with an connection which network-sets in the inactive scope
    ...
    ${resp}=     Remove Server Profile    ${create_sp_netsets3}
    Wait For Task2   ${resp}   timeout=600

    Log   Check SP5 information   console=True
    ${sp_Uri_Result}=    Get Server Profile URI     ${create_sp_netsets3["name"]}
    Should Be Equal  '${sp_uri_result}'   '/rest/server_profile_uri_${create_sp_netsets3["name"]}_not_found'
