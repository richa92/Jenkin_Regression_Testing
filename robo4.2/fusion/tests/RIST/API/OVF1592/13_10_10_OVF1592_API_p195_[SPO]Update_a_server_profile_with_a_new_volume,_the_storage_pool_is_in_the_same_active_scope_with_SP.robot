...
*** Test Cases ***
[SPO] - Update a server profile with a new volume, the storage pool is in the same active scope with SP
    [Documentation]  OVF1592_UI_p195 [Server Profile Operator] Update a server profile with a new volume, the storage pool is in the same active scope with SP
