...
class apic(object):
    ...
    def Apic_FabricNode_get(self, APPLIANCE_IP, token, IPList):
        ...
        while i <= j:
            if 'imdata' in response:
                ...
                if (id != '78:48:59:61:39:63' and id != '169.254.57.19' and id != '10.10.5.185' and id != '10.10.0.208' and id != '10.10.1.176' and id != '10.10.1.240'):
    ...
    def leaf_validate(self, switch):
        ...
        command4 = switch.command4
        BuiltIn().sleep(5)
        chan.send(command4 + '\n')
        command0 = switch.command0
        BuiltIn().sleep(5)
        chan.send(command0 + '\n')
        BuiltIn().sleep(10)
        chan.send(command1 + '\n')
    ...
    def validate_Apic_topology(self, APPLIANCE_IP, token, APIC_TOPOLOGY, NodeIp):
        # This function will get the fabric nodes and then validate the nodes Ip returned is equal to the IPList passed in this function
        import json
        import requests
        error = 0
        requestObj = HttpVerbs()
