...
def decryptor(filename, src_dir, decryptor_path, platform):
    ...
    dump_files = [filename for filename in os.listdir(src_dir) if filename.endswith(".sdmp")]
    ...
    if (platform == "linux"):
        ...
        # os.system("cd " + decryptor_path)
        if (os.system(("cd" + " " + decryptor_path) + " & " + " " + executable_filename + " " + src_file + " -k " + privatekey)):
            logger.error("Exception occurred during decryption in Windows platform")
            return False
