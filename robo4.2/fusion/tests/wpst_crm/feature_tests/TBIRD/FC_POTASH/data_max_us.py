"""
data file for testing FC max limit of uplinkset in LI and LIG
"""
...
def name_list(start, end, prefix='FA'):
    """ return list of FC network names, default prefix is FA """
    names = []
...
def get_port_name(counter1):
    """ return Potash uplink port name based on counter1 """
    print counter1
    port = (counter1 / 4 + 1)
    subport = (counter1 % 4 + 1)
    return "Q" + str(port) + "." + str(subport)


# construct portConfigInfo example as {'enclosure': '1', 'bay': '3', 'port': 'Q1.4', 'speed': 'Auto'}
def getPortConfig(counter2, enc_num):
    """ construct portConfigInfo for LIG uplinkset uplink port """
    if enc_num == 1:
        bay_num = 3
    elif enc_num == 2:
        bay_num = 6
    ...
    portconfig = "{ " + \
                 ...
                 "'bay': '" + str(bay_num) + "', " + \
                 "'port': '" + get_port_name(counter2 % 24) + "', " + \
                 "'speed': 'Auto' }"
...
def createUplinkSets(fabric_type, enc_num, count):
    """ construct uplinksets defined in LIG """
