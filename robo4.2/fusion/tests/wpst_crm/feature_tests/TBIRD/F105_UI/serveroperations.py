...
class Server:
        '''
        This class contains the code to perform io operation through Server iLO->VSP->SAC->cmd.
        Executes the diskspd command to start the traffic.
        Fetches the Diskspd results saved to the output  log file.
        Fetches the disk count available in the server
        '''
    ...
    def get_disk(self, module_file_location, getdisk_cmd):
        '''
        Starts IO traffic in the Server
        '''
        try:
            ...
            stdin, stdout, stderr = self._ssh_obj.exec_command(
                (self._pycmd + " " + self._dir_location + "/" + module_file +
                 " -i " + self._ilo_ip + " -u " + self._ilo_username + " -p " +
                 self._ilo_password + " -us " + self._server_username +
                 " -ps " + self._server_password + " -c " + getdisk_cmd))
...
def lun_discovery(linux_details, ilo_details, server_details, module_file_path, get_disk_cmd):
    '''
...
def executes(linux_details, ilo_details, server_details, module_file_path, diskspd_cmd):
    ...
    fileout = re.findall(r'([0-9][0-9]-.*.txt)', strout1)
    try:
        if not fileout:
...
if __name__ == '__main__':
    ...
    # module_file_path = "C:\\Prathiba\\fusion\\tests\\wpst_crm\\wpst-crm-india\\F118\\DiscoverLun_Linux.py"
    module_file_path = "C:\\Tamil\\fusion\\tests\\wpst_crm\\wpst-crm-india\\F118\\FetchIO_1.py"
    output_file = "11-05-2016-0941_output.txt"
    # output_file = "PowerShell.exe -ExecutionPolicy Bypass -File .\\type.ps1"
