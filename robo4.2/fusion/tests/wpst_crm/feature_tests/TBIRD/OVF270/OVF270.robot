*** Settings ***
Documentation		OVF270: TAA support for Synergy Interconnects
...					Usage: BB58 ME: pybot -V bb58_me_variables.py -v APPLIANCE_IP:15.245.131.206 OVF270.robot
...

Suite Setup   		Run FTS and test-specific setup
Suite Teardown		Teardown

Resource            ../../../../resource/fusion_api_all_resource_files.txt

Library				Collections
Library             Dialogs
*** Variables ***
${FUSION_IP}                       ${APPLIANCE_IP}
${SSH_HOST}                        ${APPLIANCE_IP}
${SSH_PASS}                        hpvse1
${VM}
${POTASH_MODEL}                    Virtual Connect SE 40Gb F8 Module for Synergy
${POTASH_MODEL_TAA}                Virtual Connect SE 40Gb F8 Module for Synergy    # Descriptions are the same
${POTASH_PART}                     794502-B23
${POTASH_PART_TAA}                 794502-B23   # Part numbers are the same

${CL10_MODEL}                      Synergy 10Gb Interconnect Link Module
${CL10_MODEL_TAA}                  Synergy 10Gb TAA Interconnect Link Module
${CL10_PART}                       779215-B21
${CL10_PART_TAA}                   779215-B22

${CL20_MODEL}                      Synergy 20Gb Interconnect Link Module
${CL20_MODEL_TAA}                  Synergy 20Gb TAA Interconnect Link Module
${CL20_PART}                       779218-B21
${CL20_PART_TAA}                   779218-B22

#TODO              Put the correct work product here!
#${RALLYWORKPRODUCT}                OVS4774
${RALLYWORKPRODUCT}                 US23937

*** Test Cases ***
Create LIG, EG, LE and verify TAA modules
    [Documentation]   Create 3 FRAME ME HA LIG (IBS2_HA_MIXED_TAA_POTASH_CL20)
    ...   ENC 1: Bay 2 Potash, Bay 6 CL20 TAA
    ...   ENC 2: Bay 3 CL20 non-TAA , Bay 6 Potash non-TAA
    ...   Ethernet only ULS using 1000 Ethernet on both Potash modules (BigPipe)
    ...   Converged ULS using 32 FCoE, 1000 Ethernet on ENC 1 Bay 3 TAA module (TAA_CONVERGED)
    ...   Create 3 FRAME ME HA LIG (IBS3_HA_MIXED_TAA_POTASH_CL10)
    ...   ENC 1: Bay 3 Potash TAA, Bay 6 CL10 TAA
    ...   ENC 2: Bay 3 CL10 non-TAA , Bay 6 Potash non-TAA
    ...   Ethernet only ULS using 1000 Ethernet on both Potash modules (BigPipe)
    ...   Converged ULS using 32 FCoE, 1000 Ethernet on ENC 1 Bay 3 TAA module (TAA_CONVERGED)
    ...   Create Enclosure Group that uses both LIGS
    ...   Create Logical Enclosure
    ...   Check that TAA modules are imported correctly and correct name and part numbers appear
    ...   Create profiles that have connections which use networks on each of the TAA modules
    [Tags]  1
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}

    ${body} =       Build LIG body      ${ligs['${LIG1}']}
	${resp} = 	    Fusion Api Create LIG	${body}
	${task} =	    Wait For Task	${resp} 	60s	2s
	${valDict} = 	Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}

    ${body} =       Build LIG body      ${ligs['${LIG2}']}
	${resp} = 	    Fusion Api Create LIG	${body}
	${task} =	    Wait For Task	${resp} 	60s	2s
	${valDict} = 	Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}

	Run Keyword for List	${enc_groups}	Add Enclosure Group from variable

	${task} =       Add Logical Enclosure from variable     ${les['${LE1}']}
	${valDict} = 	Create Dictionary	status_code=${200}
	...                                 taskState=((?i)Warning|Completed)
	Validate Response Regex	${task}	${valDict}

	${icms} =   Get IC   ${POTASH_MODEL_TAA}
	${valDict} = 	Create Dictionary	partNumber=${POTASH_PART_TAA}
	...                                 model=${POTASH_MODEL_TAA}
    Verify ICM data   ${icms}   ${valDict}

	${icms} =   Get IC   ${CL20_MODEL_TAA}
	${valDict} = 	Create Dictionary	partNumber=${CL20_PART_TAA}
	...                                 model=${CL20_MODEL_TAA}
    Verify ICM data   ${icms}   ${valDict}

    Power off all servers
	Add Server Profiles from variable   ${server_profiles}    25m    1m
	[Teardown]   Get errors

Rip and Replace TAA Interconnects
    [Documentation]   EfuseOn ENC 1 Bay 3 TAA Potash module, wait for it to become 'Absent'
    ...   EfuseOff ENC 1 Bay 3 TAA Potash module, wait for it to become 'Configured'
    ...   EfuseOn ENC 1 Bay 5 TAA CL20 module, wait for it to become 'Absent'
    ...   EfuseOff ENC 1 Bay 5 TAA CL20 module, wait for it to become 'Configured'
    ...   EfuseOn ENC 1 Bay 6 TAA CL10 module, wait for it to become 'Absent'
    ...   EfuseOff ENC 1 Bay 6 TAA CL10 module, wait for it to become 'Configured'

    [Tags]  2
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
	${icms} =   Get IC   ${POTASH_MODEL_TAA}
	Efuse ICMs    ${icms}
	${icms} =   Get IC   ${CL10_MODEL_TAA}
	Efuse ICMs    ${icms}
	${icms} =   Get IC   ${CL20_MODEL_TAA}
	Efuse ICMs    ${icms}
	[Teardown]   Get errors

Power off and on TAA Interconnects
    [Documentation]   Power Off ENC 1 Bay 3 TAA Potash module, wait for it to become 'Powered Off'
    ...   Power On ENC 1 Bay 3 TAA Potash module, wait for it to become 'Powered On'
    ...   Power Off ENC 1 Bay 5 TAA CL20 module, wait for it to become 'Powered Off'
    ...   Power On ENC 1 Bay 5 TAA CL20 module, wait for it to become 'Powered On'
    ...   Power Off ENC 1 Bay 6 TAA CL10 module, wait for it to become 'Powered Off'
    ...   Power On ENC 1 Bay 6 TAA CL10 module, wait for it to become 'Powered On'

    [Tags]  3
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
	${icms} =   Get IC   ${POTASH_MODEL_TAA}
	Power ICMs    ${icms}
	${icms} =   Get IC   ${CL10_MODEL_TAA}
	Power ICMs    ${icms}
	${icms} =   Get IC   ${CL20_MODEL_TAA}
	Power ICMs    ${icms}
	[Teardown]   Get errors

Disable and enable uplink ports on TAA Potash modules
    [Documentation]   Enable\disable an uplink port on ENC 1 Bay 3 TAA Potash module and verify state changes
    [Tags]  4
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
    ${ic_uri} =   Get IC URI     ${ENC1}, interconnect 3
	${resp} =     Get IC Port    uri=${ic_uri}   port=Q5:1
	Set to Dictionary   ${resp}   enabled    ${false}
    ${body} =    Create List
    Append to list    ${body}    ${resp}

	${resp} =     fusion api edit interconnect ports   uri=${ic_uri}   body=${body}
	${task} =       Wait For Task 	${resp} 	5 min	15s
	${valDict} =    Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}

	${resp} =     Get IC Port    uri=${ic_uri}   port=Q5:1
	Set to Dictionary   ${resp}   enabled    ${true}
    ${body} =    Create List
    Append to list    ${body}    ${resp}

	${resp} =     fusion api edit interconnect ports   uri=${ic_uri}   body=${body}
	${task} =       Wait For Task 	${resp} 	5 min	15s
	${valDict} =    Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}
	[Teardown]   Get errors

Update from group LI and verify
    [Documentation]   Change LI Uplink set (remove a network)
    ...   Issue UFG LI
    ...   Verify the uplink set returns to correct state
    [Tags]  5
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
	${li_uri} = 	Get LI URI   ${LE1}-${LIG2}
	${us} = 		Copy Dictionary	${li_uplink_sets['${LIG2}${US1}']}
	${body} = 		Build US body 	${us}	${li_uri}
	${us_uri} = 	Get Uplinkset URI  	${LIG2}${US1}
	${resp} = 		Fusion Api Edit Uplink Set	body=${body}	uri=${us_uri}
	${task} =       Wait For Task 	${resp} 	15 min	15s
	${valDict} = 	Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}
    ${resp} =       fusion api update from group    ${li_uri}
	${task} =       Wait For Task 	${resp} 	15 min	15s
	${valDict} =    Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}
	[Teardown]   Get errors

Update from group LE and verify
    [Documentation]   Change LI Uplink set (add an uplink port)
    ...   Issue UFG LE
    ...   Verify the uplink set returns to correct state
    [Tags]  6
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
	${li_uri} = 	Get LI URI   ${LE1}-${LIG2}
	${us} = 		Copy Dictionary	${li_uplink_sets['${LIG2}${US2}']}
	${body} = 		Build US body 	${us}	${li_uri}
	${us_uri} = 	Get Uplinkset URI  	${LIG2}${US2}
	${resp} = 		Fusion Api Edit Uplink Set	body=${body}	uri=${us_uri}
	${task} =       Wait For Task 	${resp} 	15 min	15s
	${valDict} = 	Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}
	${le_uri} =     Get LE URI    ${LE1}
    ${resp} =       fusion api update logical enclosure from group   uri=${le_uri}
	${task} =       Wait For Task 	${resp} 	40 min	15s
	${valDict} =    Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}
	[Teardown]   Get errors

Remove LE and verify
    [Documentation]   Remove the Logical Enclosure and verify it is removed successfully
    [Tags]  7
    [Setup]    Run Keyword and Ignore Error    Write To ciDebug Log
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
    Power off all servers
    Remove All Server Profiles

    ${resp} =       fusion api delete logical enclosure    name=${LE1}
	${task} =       Wait For Task 	${resp} 	15 min	5s
	${valDict} = 	Create Dictionary	status_code=${200}
	...                                 taskState=Completed
	Validate Response	${task}	${valDict}
	[Teardown]   Get errors

*** Keywords ***
Get Errors
    ${ERRORS}=   Run Keyword and Ignore Error    Get Test Errors from ciDebug Log     ${TEST_NAME}

Verify ICM data
    [Arguments]     ${icms}   ${valDict}
	:FOR	${ic}	IN      @{icms}
	\ 	Validate Response	${ic}	${valDict}

Teardown
	Set Log Level	TRACE
	Log to console and logfile	[TEARDOWN]
	Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
	Power off ALL Servers
	Remove All Server Profiles
	Remove ALL LS
	Remove ALL LSGs
	Remove All Logical Enclosures
	Remove ALL Enclosure Groups
	Remove ALL LIGs
	Remove ALL Ethernet Networks
	Remove ALL FC Networks
	Remove ALL FCoE Networks
	Remove ALL Network Sets
	Remove ALL Users

Run FTS and test-specific setup
	Set Log Level	TRACE
    Run Keyword If	${appliance} is not ${null}   FTS
    Test Specific Setup

Set Metadata
    Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
    ${resp} =    Fusion Api Get Appliance Version
    Set Suite Metadata     OneView Version      ${resp['softwareVersion']}    top=True
	Set Suite Metadata     rallyWorkproduct      ${RALLYWORKPRODUCT}    top=True

FTS
    [Tags]  FTS
	Set Log Level	DEBUG
	log variables
    Get VM IP   ${VM}
	First Time Setup    password=hpvse123    interface=bond0

Test Specific Setup
    [Tags]  TSS     Setup
	Set Log Level	TRACE
	Set Suite Metadata     rallyWorkproduct      ${RALLYWORKPRODUCT}    top=True
	Run Keyword and Ignore Error    Write To ciDebug Log
	Log to console and logfile	[TEST-SPECIFIC SETUP]
    Fusion Api Login Appliance 		${APPLIANCE_IP}		${admin_credentials}
	${users} =	Get Variable Value	${users}
	Run Keyword If	${users} is not ${null}	Add Users from variable				${users}
	${ethernet_networks} =	Get Variable Value	${ethernet_networks}
	Run Keyword If	${ethernet_networks} is not ${null}	Add Ethernet Networks from variable	${ethernet_networks}
	${network_sets} =	Get Variable Value	${network_sets}
	Run Keyword If	${network_sets} is not ${null}	Add Network Sets from variable		${network_sets}
	${fc_networks} =	Get Variable Value	${fc_networks}
	Run Keyword If	${fc_networks} is not ${null}	Add FC Networks from variable		${fc_networks}
	${fcoe_networks} =	Get Variable Value	${fcoe_networks}
	Run Keyword If	${fcoe_networks} is not ${null}	Add FCoE Networks from variable		${fcoe_networks}
	${ranges} =	Get Variable Value	${ranges}
	${pools} =  Run Keyword If	${ranges} is not ${null}	Create List		/rest/id-pools/vmac	/rest/id-pools/vwwn	/rest/id-pools/vsn
    Run Keyword If	${ranges} is not ${null}                Run Keyword for List	${pools}	Disable ALL Generated ID Ranges
	Run Keyword If	${ranges} is not ${null}				Add Ranges From variable	${ranges}
	Power Off All Servers

Efuse ICMs
    [Arguments]     ${icms}
	Set Log Level	TRACE
	${l} = 	Get Length	${icms}
	:FOR	${x}	IN RANGE	0	${l}
    \   ${ic} =     Get From List   ${icms}    ${x}
    \   Log to console      ${ic['name']} ${ic['uri']}
    \   ${enc} =    Fetch from left     ${ic['name']}    ,
    \   ${bay} =    Fetch from right    ${ic['name']}    ${SPACE}
    \   Get EM IP       ${enc}
    \   Get EM Token    ${enc}
	\   fusion_api_appliance_setup.Log to console and logfile  	\t Waiting for ICM in Bay:${bay} to reach state:Configured|Monitored
    \   Wait Until Keyword Succeeds     3 min   5s      IC reached state    ${ic['uri']}    Configured|Monitored
    \   Run Keyword and Ignore Error    fusion_api_appliance_logging.Write To ciDebug Log    [EFUSE ON via RIS (REMOVE ICM)]
    \   Efuse ICM   EFuseOn     ${bay}
	\   fusion_api_appliance_setup.Log to console and logfile  	\t Waiting for ICM in Bay:${bay} to reach state:Absent
    \   Wait Until Keyword Succeeds     10 min   15s      IC reached state    ${ic['uri']}    Absent
    \   Run Keyword and Ignore Error    fusion_api_appliance_logging.Write To ciDebug Log    [EFUSE OFF via RIS (INSERT ICM)]
    \   Efuse ICM   EFuseOff    ${bay}
	\   fusion_api_appliance_setup.Log to console and logfile  	\t Waiting for ICM in Bay:${bay} to reach state:Configured|Monitored
    \   Wait Until Keyword Succeeds     35 min   15s      IC reached state    ${ic['uri']}    Configured|Monitored

Power ICMs
    [Arguments]     ${icms}
    Set Log Level	TRACE
    ${icm_len} = 	Get Length	${icms}
    :FOR	${x}	IN RANGE	0	${icm_len}
    \   ${ic} =     Get From List   ${icms}    ${x}
    \   Log to console      ${ic['name']} ${ic['uri']}
    \   ${uri} =    Get From IC     ${ic}   uri
    \   Power request    ${uri}   Off
	\   fusion_api_appliance_setup.Log to console and logfile  	\t Waiting for ${uri} to reach state:Maintenance
    \   Wait Until Keyword Succeeds     60 min   30s      IC reached state    ${ic['uri']}    Maintenance
    \   Power request    ${uri}   On
	\   fusion_api_appliance_setup.Log to console and logfile  	\t Waiting for ${uri} to reach state:Configured
    \   Wait Until Keyword Succeeds     60 min   30s      IC reached state    ${ic['uri']}    Configured

Power request
    [Arguments]     ${uri}    ${power}
    ${data} =   Create Dictionary   op=replace
    ...                             path=/powerState
    ...                             value=${power}
    ${body} =   Create List     ${data}
    Run Keyword and Ignore Error    Write To ciDebug Log    \nPowering ${power}: ${uri}
    ${resp} =   fusion api patch interconnect   body=${body}    uri=${uri}
    ${task} =   Wait for Task   ${resp}  5m   10s
    ${valDict} =    Create Dictionary       status_code=${200}
    ...                                 taskState=Completed
    Validate Response       ${task}   ${valDict}
	fusion_api_appliance_setup.Log to console and logfile  	\t Waiting for ${uri} to reach powerState:${power}
    Wait Until Keyword Succeeds     10 min   20s      IC reached powerState    ${uri}    ${power}

IC reached state
	[Arguments]	    ${uri}  ${state}
	Set Log Level	TRACE
    ${resp} =   fusion api get resource     ${uri}
	fusion_api_appliance_setup.Log to console and logfile  	\t ${uri}: ${resp['state']}
	Should Match Regexp 	${resp['state']}    ${state}
	[Return]	${resp}

IC reached powerState
	[Arguments]	    ${uri}  ${state}
	Set Log Level	TRACE
    ${resp} =   fusion api get resource     ${uri}
	fusion_api_appliance_setup.Log to console and logfile  	\t ${uri}: ${resp['powerState']}
	Should Match Regexp 	${resp['powerState']}    ${state}
	[Return]	${resp}

Get IC
	[Arguments]	    ${model}
    ${resp} =   fusion api get interconnect
    ${ic_list} =    Create List
    ${ics} =     Get From Dictionary     ${resp}    members
	${l} = 	Get Length	${ics}
	:FOR	${x}	IN RANGE	0	${l}
    \   ${ic} =     Get From List   ${ics}    ${x}
	\ 	Run Keyword If 	'${ic['model']}' != '${model}'		Continue For Loop
	\   Append to list      ${ic_list}  ${ic}
    #${ordered_list} =   helper.order_icms    ${ic_list}     ${ICM_ORDER}

    [Return]    ${ic_list}

Get IC Port
	[Arguments]	    ${uri}  ${port}
    ${resp} =   fusion api get interconnect ports    uri=${uri}
    ${ics} =     Get From Dictionary     ${resp}    members
	:FOR	${ic}	IN      @{ics}
	\ 	${return} =    Run Keyword If 	'${ic['portName']}' == '${port}'		set variable     ${ic}
	\ 	Exit for loop if  	'${ic['portName']}' == '${port}'
    [Return]    ${return}


Get from IC
    [Arguments]     ${ic}   ${element}
    ${return} =     Get From Dictionary     ${ic}   ${element}
    [Return]    ${return}


Add key value to header
    [Documentation]  adds key\value pairs to the session default headers and returns the resulting dict
    [Arguments]     &{kwargs}
    ${headers} =    Fusion API get headers
	:FOR	${key}   ${value}	IN   @{kwargs}
	\	Set to Dictionary   ${headers}   ${key}   ${value}
    [Return]    ${headers}

