...
Documentation       OVF271 - SuiteName : Configure and Validatin the bi-directional port mirroring for carbon with Aside configuration
...
3_Enable Port Monitoring with Unauthorized User
    ...
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor}   ${interconnects[0]}    ${analyzer_port_qport}    true   ${LI_A}
    Log to Console  ${resp}
...
5_Configure Port Monitoring with Analyzer port as Q1_4 and Monitored port as 1 with direction as bi-directional
    ...
    Verify Port Monitoring in IC        ${analyzer_port_qport}  ${analyzer_dport}       ${interconnects[0]}     ${li_portmonitor['monitoredPorts'][0]['portMonitorConfigInfo']}

    sleep   20
    # Verify the port mirroring is configured properly in the interconnect CLI

    verify port mirroring in ICM CLI        ${interconnects[0]}     ${ic_bay1}  ${analyzer_port_qport}

6_Negative : Create or edit an Uplinkset and try to use the analyzerPort Q1:4 and verify the error message
    [Documentation]     Negative : Create or edit an Uplinkset and try to use the analyzerPort Q1:4 and verify the error message
    ${li_uri} =     Get LI URI   ${LI_A}
...
7_Negative : Configure port monitoring with an uplink port [Q1:1] which is already in the uplink set
    [Documentation]     Negative : Configure port monitoring with an uplink port [Q1:1] which is already in the uplink set
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor}   ${interconnects[0]}    ${used_uplink_port}     true     ${LI_A}
    Log to Console      ${resp}
...
8_Disable the analyzer port Q1:4, and verify the port monitoring is disbaled
    ...
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor}   ${interconnects[0]}    ${analyzer_port_qport}    false      ${LI_A}
    Run Keyword If  ${resp['status_code']} !=202    fail    Failed to Disable Port Monitor for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
...
9_Configure Port monitoring by switching from the port Q1_4 to Q1_3
    [Documentation]     Configure Port monitoring by switching from the port Q1.4 to Q1:3
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor}   ${interconnects[0]}    ${analyzer_qport3_bay1}    true      ${LI_A}
...
10_Disable the Port monitoring configuration on the Analyzer port Q1_3
    [Documentation]     Disable the Port monitoring configuration on the Analyzer port Q1.3
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor}   ${interconnects[0]}    ${analyzer_qport3_bay1}    false     ${LI_A}
    Run Keyword If  ${resp['status_code']} !=202    fail    Swirching to port & Enabling Port Monitor is failed for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
    ...         ELSE    Log to console and logfile  \n Successfully configured the port monitoring !!

11_Configure Port monitoring by choosing the "to server" option for the Port Q1_4 in bay 1
    [Documentation]     Configure Port monitoring by choosing the "to server" option for the Port Q1.4 in bay 1
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor_to_server}     ${interconnects[0]}    ${analyzer_port_qport}    true       ${LI_A}
    Run Keyword If  ${resp['status_code']} !=202    fail    Swirching to port & Enabling Port Monitor is failed for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
    ...         ELSE    Log to console and logfile  \n Successfully configured the port monitoring !!!
    ...
    # Verify the port is successfully configured as analyzer port with direction as bi-directional
     ...
     Wait Until Keyword Succeeds    20 min   15s    Verify Port Monitoring in IC        ${analyzer_port_qport}  ${analyzer_dport}       ${interconnects[0]}     ${li_portmonitor_to_server['monitoredPorts'][0]['portMonitorConfigInfo']}

    # Verify the port mirroring is configured properly in the interconnect CLI
    sleep   20
     Wait Until Keyword Succeeds    20 min   15s    verify port mirroring in ICM CLI        ${interconnects[0]}     ${ic_bay1}  ${analyzer_port_qport}

12_Configure Port monitoring by choosing the "from server" option for the Port Q1:3 in bay 1
    [Documentation]     Configure Port monitoring by choosing the "from server" option for the Port Q1:3 in bay 1
    ${resp}=    Configuring Port Monitoring in LI   ${li_portmonitor_from_server}   ${interconnects[0]}    ${analyzer_qport3_bay1}    true      ${LI_A}
    ...
    # Verify the port is successfully configured as analyzer port with direction as bi-directional
     ...
     Wait Until Keyword Succeeds    20 min   15s    Verify Port Monitoring in IC        ${analyzer_qport3_bay1} ${analyzer_dport}       ${interconnects[0]}     ${li_portmonitor_from_server['monitoredPorts'][0]['portMonitorConfigInfo']}
...
Get Server iLO IP
    [Documentation]     Get Server iLO IP
...
Suite Setup tasks
    ...
    Create LIG and Check Status     ${body}     ${Lig_name_A}

    # Verify the Uplinksets are present in the LIG

    ${resp} =    Fusion Api Get Lig     param=?filter="'name'=='${ligs['lig2']['name']}'"
    Log to Console      ${resp}
    ...
    Add Enclosure Group from variable   ${enc_groups['enc_group_Aside']}

    Add Logical Enclosure from variable   ${les['le2']}

    # Creating server profiles and poweron the servers

    ${resp}     Add Server Profiles from variable   ${server_profiles_A_side}
    Log to Console      ${resp}
    Power on server    ${server_profiles_A_side[0]['serverHardwareUri']}
    ${resp}     Add Server Profiles from variable   ${server_profiles2_A_side}
    Log to Console      ${resp}
    Power on server    ${server_profiles2_A_side[0]['serverHardwareUri']}
    ${resp}     Add Server Profiles from variable   ${server_profiles3_A_side}
    Log to Console      ${resp}
    Power on server    ${server_profiles3_A_side[0]['serverHardwareUri']}
