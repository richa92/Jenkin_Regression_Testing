*** Settings ***
Documentation		Validating the Port Monitoring for F110 with Potash and Chloride 10 with redundancy as HA
...
3. HA Enable Port Monitoring with Unauthorized User

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${analyzer_port}    true
    Log to Console  ${resp}
    Run Keyword If  ${resp['status_code']} !=401    fail    Warning !!! Unauthorized user have access for Port Monitor ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    
    
4. HA Log-out from the appliance
...
5. HA Positive : Enable Port Monitoring with Authorized User using port Q2.3
...
6. HA Negative : Create Uplinkset using the analyzerPort Q2.3
...
7. HA Negative : Enable Port Monitoring with uplink port in the Uplinkset

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${used_uplink_port}     true
    Run Keyword If  ${resp['status_code']} ==202    fail    Enabling Port Monitor is succeed for Uplinkport for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    Sleep      ${pm_timer}


8. HA Negative : Enable Port Monitoring with an uplink port that is an Fiber channel US using port "Q1"

   ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${fc_uplink_port}   true
   Log to Console      ${resp}     ${\n}   
   Run Keyword If  ${resp['status_code']} !=400    fail   Enabling Port Monitor is Success for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}


9. HA Negative : Enable Port Monitoring with a stacking port as the analyzerPort

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${stacking_port}    true
    Run Keyword If  ${resp['status_code']} ==202    fail    As Expected Enabling Port Monitor is failed for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    Sleep      ${pm_timer}
    
10. HA Positive : Disable Port Monitoring with Authorized Users

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${analyzer_port}    false
    Run Keyword If  ${resp['status_code']} !=202    fail    Failed to Disable Port Monitor for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
    ...         ELSE    Log to console and logfile  \n Successfully configured the port monitoring !!!

11. HA Switch from ports Q2.3 to Q6

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${analyzer_port_2}    true
...
12. HA Disable Port Monitoring with the port Q6

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${analyzer_port_2}    false
    Run Keyword If  ${resp['status_code']} !=202    fail    Swirching to port & Enabling Port Monitor is failed for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
    ...         ELSE    Log to console and logfile  \n Successfully configured the port monitoring !!!
    
13. HA Switch the ports from Port Q6 to Q2.4

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${analyzer_port_3}    true
    Run Keyword If  ${resp['status_code']} !=202    fail    Swirching to port & Enabling Port Monitor is failed for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
    ...         ELSE    Log to console and logfile  \n Successfully configured the port monitoring !!!
...
14. HA Disable the port monitoring with port Q4.1

    ${resp}=    Configuring Port Monitoring in LI   ${interconnects[0]}    ${analyzer_port_3}    false
    Run Keyword If  ${resp['status_code']} !=202    fail    Swirching to port & Enabling Port Monitor is failed for ${LI}\nErrorCode:${resp['errorCode']}\nMessage:${resp['message']}
    ${task} =   Wait For Task   ${resp}     ${pm_timer_q5}    2s
    Run Keyword If  '${task['taskState']}' !='Completed' or ${task['status_code']} !=200    fail    Configure port monitoring creation failed\nTaskErrorCode:${task['taskErrors'][0]['errorCode']}\nTaskStatus:${task['taskErrors'][0]['message']}
    ...         ELSE    Log to console and logfile  \n Successfully configured the port monitoring !!! 
    
15. HA Switch the ports from Port Q2.4 to Q2.3
...
16. HA Disable the port monitoring with port Q4.4
...
Suite Setup tasks
	...
	${lig_body}=	set_networkuri_lig	${lig_tbird_Enc${frame}}	${network}
	${fc_network}=	Fusion Api Get FC Networks
	${lig_body}=	set_networkuri_lig	${lig_tbird_Enc${frame}}	${fc_network}
