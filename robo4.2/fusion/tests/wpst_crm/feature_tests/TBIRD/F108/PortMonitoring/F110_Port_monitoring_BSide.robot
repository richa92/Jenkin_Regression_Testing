...
1. Bside Create Server Profiles
...
2. Bside Log-in as Server Admin
...
3. Bside Enable Port Monitoring with Unauthorized User
...
4. Bside Log-out from the appliance
...
5. Bside Positive : Enable Port Monitoring with Authorized User using port Q2.3
...
6. Bside Negative : Create Uplinkset using the analyzerPort Q2.3
...
7. Bside Negative : Enable Port Monitoring with uplink port in the Uplinkset
...
8. Bside Negative : Enable Port Monitoring with an uplink port that is an Fiber channel US using port "Q1"
...
9. Bside Negative : Enable Port Monitoring with a stacking port as the analyzerPort
...
10. Bside Positive : Disable Port Monitoring with Authorized Users
...
11. Bside Switch from ports Q2.3 to Q6
...
12. Bside Disable Port Monitoring with the port Q6
...
13. Bside Switch the ports from Port Q6 to Q2.4
...
14. Bside Disable the port monitoring with port Q4.1
...
15. Bside Switch the ports from Port Q2.4 to Q2.3
...
16. Bside Disable the port monitoring with port Q4.4
...
Suite Setup Tasks
	...
	${lig_body}=	set_networkuri_lig	${Lig_tbird_Bside_Enc${frame}}	${network}
	${fc_network}=	Fusion Api Get FC Networks
	${lig_body}=	set_networkuri_lig	${Lig_tbird_Bside_Enc${frame}}	${fc_network}
	#Log to Console	${lig_body}	${\n}
	${ligs}=  Create LIG TBird Payload   ${lig_body}
	Log to Console  ${ligs} ${\n}
	${resp}=  fusion api create lig     ${ligs}
