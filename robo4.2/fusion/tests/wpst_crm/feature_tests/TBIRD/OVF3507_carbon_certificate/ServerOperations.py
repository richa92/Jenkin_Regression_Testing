...
class Server:
    ...
    def perform_io(self, module_file_location, diskspd_cmd):
        ...
        try:
            ...
            build_cmd = (self._pycmd + " " + self._dir_location + "/" + module_file +
                         " -i " + self._ilo_ip + " -u " + self._ilo_username + " -p " +
                         self._ilo_password + " -us " + self._server_username +
                         " -ps " + self._server_password + " -c " + diskspd_cmd)
            print ("%s" % build_cmd)
            stdin, stdout, stderr = self._ssh_obj.exec_command(build_cmd)
            time.sleep(40)
            print ("%s %s %s" % (stdin, stdout, stderr))
            output = stdout.readlines()
            return output
        except Exception as e:
            print (e)

    def fetch_io(self, module_file_location):
        '''
...
def executes(linux_details, ilo_details, server_details, module_file_path, diskspd_cmd):
    ...
    print ("Logged into Linux Machine")
...
def execute_traffic(linux_details, ilo_details, server_details, module_file_path, diskspd_cmd):
    io_thread = threading.Thread(target=executes, args=(linux_details, ilo_details, server_details, module_file_path, diskspd_cmd))
    io_thread.start()


def ioresults(linux_details, ilo_details, server_details, module_file_path):
    '''
    ...
    serv_obj = Server(linux_details, ilo_details, server_details)
    serv_obj.connect_linux_machine()
    output = serv_obj.fetch_io(module_file_path)
    output_dict = {}
    print("%s" % output)
    strout = str(output[0])
