...
def execute_windows_commands(ip, username, passwd, wcmd):
    ...
    '''
    try:
        single_cmd = "C:\\PSTOOLS\\PSTools\\paexec \\\\" + ip + " -u " + username + " -p " + passwd + " " + \
                     wcmd
        print single_cmd
...
def execute_traffic(server_details):
    threads = [threading.Thread(target=execute_windows_commands, args=(i['ip'], i['userName'], i['password'], i['wcmd']))
...
def Kill_taffic(server_details, kill_cmd):
    threads = [
