...
*** Test Cases ***
1_Create Networks, LIG, EG and import enclosure through API
    [Documentation]    Creating Networks, LIG, EG and importing enclosure
...
4_Get the interconnect and port statistics details
    [Documentation]    Fetching Interconnect port statistics
    ...
    ${output}   ${msg}=     executes        ${linux_details}    ${oa_details_3}    ${module_file_path}     ${diskspd_cmd}  ${windows_server_cred}
...
6_Power off all servers
    [Documentation]    Power off servers
    Power off ALL servers

7_Delete the Server profile
    [Documentation]    Deleting the server profile
...
8_Delete the Enclosure
    [Documentation]    Removing enclosure cleanup the setup
...
9_Delete the Networks
    [Documentation]    Delete the networks
