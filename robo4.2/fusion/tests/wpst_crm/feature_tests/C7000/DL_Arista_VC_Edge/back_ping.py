...
def checkcontinuousping(ip, name):
    print "dddddd start"
    print os.system("ping -t %s > %s" % (ip, name))
    time.sleep(2)


def startpingthread(ip, name):
    t1 = threading.Thread(name="checkcontinuousping", target=checkcontinuousping, args=(ip, name))
    # name=logger1.txt
    t1.start()
