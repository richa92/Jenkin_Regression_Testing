...
diskspd_cmd, server_cred ):
	...
	oa_obj = OAOperations()
	effuse_thread = threading.Thread(target = oa_obj.efuse_device,args = \
	(oa_details['oa_ip'],oa_details['username'],oa_details['password'],\
	'oa', '1','off'))
	failover_thread = threading.Thread(target =oa_obj.forceFailOver, args=\
	(oa_details['oa_ip'],oa_details['username'],oa_details['password']))
	io_thread.start()
	time.sleep(20)
	effuse_thread.start()
...
def execute_IO_and_disable_enable_ports(*args):
	...
	while iter <  205:
		if iter %2 == 0:
			port_thread = threading.Thread(target =\
			oa_obj.disable_enable_interconnect_ports,\
			args=(uri, output,portno[0], 'true' ))
			port_thread_1 = threading.Thread(target =\
			oa_obj.disable_enable_interconnect_ports,\
