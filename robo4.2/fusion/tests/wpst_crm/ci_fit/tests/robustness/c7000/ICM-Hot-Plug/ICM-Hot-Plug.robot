*** Settings ***
Documentation		ICM-Hot-Plug.robot  -  uses OA-based eFuse to add-remove ICMs of specified enclosure and check that they return to the expected state for x number of cycles.
...                     Example:
...                              time pybot -T -d /tmp/logs/ICM-Hot-Plug.robot -LTRACE -vENC:CI-FIT-16 -V /root/ci-fit/config_files/ci-fit-xx_test_data_variables.py ICM-Hot-Plug.robot
...
...
...
...                     Required argument:
...                         -V /root/ci-fit/config_files/ci-fit-xx_test_data_variables.py
...
ICM Off-On and check state Absent-Configured
    ...
    Run Optional Data Compare    ${GOLDEN_FILE}   preICMHotPlug.json
    Cycle    ${CYCLES}   ${icms_uri}
    Run Optional Data Compare    ${GOLDEN_FILE}   postICMHotPlug.json
...
Cycle
    ...
    \   Process ICMs For C7000 ICM Hot Plug   ${icms_uri}   state_timeout=${ICM_HOTPLUG_STATE_WAIT_TIMEOUT}   state_interval=${ICM_HOTPLUG_STATE_WAIT_INTERVAL}   remove_sleep=${POST_ICM_REMOVE_SLEEP}   cycle_sleep=${ICM_HOTPLUG_CYCLE_SLEEP}
    \   Check Common Resource Attributes
    \   Bonding Interfaces Should Be In Expected States   ${Null}   ignoreAttributes=${IGNORE_BOND_ATTRIBUTES}
    \   Run Keyword If   ${REMOTE_RUN_CHECKS} != @{EMPTY}   Run Remote Check Servers Script   ${REMOTE_RUN_CHECKS}   offline_interconnect=${Null}   tags=${REMOTE_CHECKS_TAG}
