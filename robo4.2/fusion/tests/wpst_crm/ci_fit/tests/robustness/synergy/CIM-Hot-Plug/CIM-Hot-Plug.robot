*** Settings ***
Documentation		CIM-Hot-Plug  -  Perform active CIM Hot Plug and check that they return to the expected state.
...                     Example:
...                              pybot -T -d /tmp/logs/CIM-Hot-Plug -LTRACE -V /root/ci-fit/config_files/eaglexx_test_data_variables.py CIM-Hot-Plug.robot
...                     Required argument:
...                         -V /root/ci-fit/config_files/eaglexx_data_variables.py
...                         -v HA_FILE:/path/to/your/tcs/HA_file.conf
...                     Optional arguments:
...                         To trigger detailed resource checking, supply -vGOLDEN_FILE:<golden resource data json> (Run test Generate-Resource-Json.robot to create golden file)
...
@{ignoreList}                   uuid  virtualUuid  installedStateTimestamp  portStatusReason  romVersion  uidState  refreshState  roles  ipAddressType  ipAddress  remoteSupportSettings  assetTag  enclosureUris_ADD_refreshState  deviceUri_ADD_refreshState  portTypeExtended  hpSmartUpdateToolStatus  connectorType  interconnectIP  linkPortIsolated  remotePortDescription  remoteSystemDescription  deviceUri_ADD_romVersion  deviceUri_ADD_assetTag  installState  personalityChecksum  powerAvailableWatts  powerCapacityBoostWatts  serverHwChecksum  mpFirmwareVersion  portId  lagId  serverSettings
...
Cycle
    ...
    \   ${CIMBayList} =   Get CIManager Populated Bays
    \   Perform CIMs Efuse   ${CIMBayList}   sleep=${POST_CIM_HOTPLUG_SLEEP}
    \   Check Common Resource Attributes
    \   Bonding Interfaces Should Be In Expected States   ${Null}   ignoreAttributes=${IGNORE_BOND_ATTRIBUTES}
    \   Run Keyword If   ${REMOTE_RUN_CHECKS} != @{EMPTY}   Run Remote Check Servers Script   ${REMOTE_RUN_CHECKS}   offline_interconnect=${Null}   tags=${REMOTE_CHECKS_TAG}
