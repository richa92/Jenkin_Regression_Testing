*** Settings ***
Documentation		ICM-EFuseReset.robot  -  uses EM based eFuse to reset ICMs and check that they return to the expected state
...                     Example:
...                              pybot -T -d /tmp/logs/ICM-EFuseReset -LTRACE -V /root/ci-fit/config_files/eaglexx_data_variables.py ICM-EFuseReset.robot
...                     Required argument:
...
EFuseReset ICM and check state Absent-Configured
...
Cycle
    ...
    \   Process ICMs For Synergy ICM Efuse Reset   ${icms}   wait_time=${ICM_EFUSE_RESET_WAIT_TIMEOUT}   wait_interval=${ICM_EFUSE_RESET_WAIT_INTERVAL}   cycle_sleep=${ICM_EFUSE_RESET_CYCLE_SLEEP}   iface=${TCS_HAFNIUM_SSH_INTERFACE}
