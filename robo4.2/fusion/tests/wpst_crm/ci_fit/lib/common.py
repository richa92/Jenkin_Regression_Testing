...
class connection(object):
    ...
    def __init__(self, ip, sessionId, api="600"):
        urllib3.disable_warnings()
        self._sessionId = CicAuth(sessionId)
        # self._sessionId = sessionId
        self._cred = None
        self._ip = ip
        self._apiVersion = api
...
class TaskBase(object):
    ...
    def waitTask(self, uri, interval=2):
        ...
        while running:
            ...
            if count == 0:
                if task['taskState'] != 'Completed':
                    logger._log_to_console_and_log_file(" Task: [%s:%s] is: %s\t%s" % (task['name'], task['category'], task['taskState'].ljust(10), resource))

            for x in sorted(task['progressUpdates'], key=itemgetter('id')):
                if int(x['id']) > maxId:
                    if x['statusUpdate']:
                        logger._log_to_console_and_log_file(" Task: [%s:%s] is: %s\tStatus: %s" % (task['name'], task['category'], task['taskState'].ljust(10), x['statusUpdate']))
                    maxId = int(x['id'])
