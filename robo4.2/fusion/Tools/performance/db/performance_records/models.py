...
class FusionResource(models.Model):
    ...
    def __str__(self):              # __unicode__ on Python 2
        string = "[" + str(self.id) + "] " + self.action + " " + self.resource
...
class FirmwareVersion(models.Model):
    ...
    def __str__(self):
        string = "[" + str(self.id) + "] " + self.fusion_release + " " + self.platform + " " + self.feature_set
        if self.delta != '':
            string += " + " + self.delta
        return string
...
class PerformanceRecord(models.Model):
    ...
    def __str__(self):
        string = "[" + str(self.id) + "] " + " " + str(self.resource_id) + ": " + str(self.duration) + "ms"
        return string
