...
# Usage Example:
#     pybot --listener  FusionLibrary.performance.listener  -v APPLIANCE_IP:16.x.x.x  test.robot
#
##############################################################
...
# OneView task output is written as newline-delimited JSON
# to be consumable by elastic search
# see http://ndjson.org/
OV_TASK_FILENAME = 'oneview_tasks.ndjson'
DEFAULT_PERFORMANCE_LOGGING_SERVER = 'http://rist-elk.vse.rdlabs.hpecorp.net:9200'
DEFAULT_PERFORMANCE_INDEX_NAME = "ov_perf"
...
def write_oneview_tasks(fout, task_tree, condition):
    """
    Iterate items in the task tree and write them to Elk and ndjson
    """

    def delete_lists_from_task(task):
        """
            Helper function to delete list elements from the task since
            ELK doesn't like them.
        """
        to_del = []
    ...
    # write extra informational data to the task
    metadata = BuiltIn().get_variable_value("&{SUITE METADATA}")
    test_info = {
        'suiteName': BuiltIn().get_variable_value("${SUITE_NAME)"),
        'suiteSource': BuiltIn().get_variable_value("${SUITE_SOURCE)"),
        'testName': BuiltIn().get_variable_value("${TEST_NAME)"),
        'oneViewVersion': metadata["OneView Version"] if "OneView Version" in metadata else None
    }
    # if the task tree has elements, descend into the resources and write them
    if task_tree:
        if "resource" in task_tree:
            data = task_tree["resource"]

            # remove child lists from the task data because of Elk
            delete_lists_from_task(data)
            # add the additional metadata to the task info
            data.update(test_info)
            # calculate the duration
            data['duration'] = (
                parse_date(data['modified']) - parse_date(data['created'])).total_seconds()

            # get the threshold data and add to data
            associated_resource = data.get('associatedResource')
            if associated_resource:
                ...
                data['isWithinThreshold'] = async_results
            data['condition'] = "-".join(condition)

            # place the data onto the queue to be written to Elk
            if not data.get('parentTaskUri'):
                task_queue.put(ElkItem("task", data))
            else:
                task_queue.put(ElkItem("subTask", data))

            # write the perf index line and data line
            # NOTE: this is ndjson so no commas needed
            with open(fout, "a+") as f:
                f.write(
                    '{{"index": {{"_index": "{}", "_type": "performance"}}}}\n'.format(
                        BuiltIn().get_variable_value(
                            "${PERFORMANCE_INDEX_NAME)").lower()
                    )
                )
                f.write(json.dumps(data) + "\n")
        # if there are child tasks, enumerate them as well
        if "children" in task_tree:
            for child in task_tree["children"]:
                write_oneview_tasks(fout, child, condition)
...
def get_performance_async_results(resource_name, total_elapsed, condition):
    """
    This function returns the ato {}synchronous performance results(True or False) based on the resource(sever profile, enclosure,etc) and conditions.
...
def start_suite(name, attrs):
    """
    This function fires when a test suite starts.
    It uses the first suite in order to set up the performance
    listener variables and initialize the OV task perf json file.
    """
    global output
    global task_json_file
    global queue_writer

    # if this is suite 1, initialize
    if attrs['id'] == 's1':
        # set up the task json filename
        output = BuiltIn().get_variable_value("${OUTPUTDIR}")
        task_json_file = os.path.join(output, OV_TASK_FILENAME)

        # set the perf index name if not already set
        if BuiltIn().get_variable_value("${PERFORMANCE_INDEX_NAME}") is None:
            BuiltIn().set_global_variable(
                "${PERFORMANCE_INDEX_NAME}", DEFAULT_PERFORMANCE_INDEX_NAME)
        if BuiltIn().get_variable_value("${PERFORMANCE_LOGGING_SERVER}") is None:
            BuiltIn().set_global_variable(
                "${PERFORMANCE_LOGGING_SERVER}", DEFAULT_PERFORMANCE_LOGGING_SERVER)
        logger.info("Performance listener enabled")

        # create a new njson file
        with open(task_json_file, "w+") as fout:
            fout.truncate()
        # create the queue writer thread and start it
        host = BuiltIn().get_variable_value("${PERFORMANCE_LOGGING_SERVER}")
        index = BuiltIn().get_variable_value("${PERFORMANCE_INDEX_NAME}")
        logger.info("Elk server:     {}".format(host))
        logger.info("Elk index :     {}".format(index))
        queue_writer = ElkQueueWriter(host, index, task_queue)
        logger.info("Elk version:    {}".format(queue_writer.get_elk_version()))
        queue_writer.start()
...
def end_suite(name, attrs):
    global task_queue
    global queue_writer
    if attrs['id'] == 's1' and queue_writer:
        start = datetime.datetime.now()
        if not task_queue.empty():
            logger.debug("Waiting for Elk queue writer")
        while not task_queue.empty() and (datetime.datetime.now() - start).total_seconds() < 10.0:
            sleep(1)
...
def end_keyword(name, attrs):
    ...
    if performance:
        ...
        if 'fusion api' in kwname:
                    ...
                    if assign_var:
                        ...
                        if post_resp:
                            ...
                            if task_uri:
                                ...
                                task_list.append(task_details.copy())
                    if condition is not None:
                        condition_resource = condition[0].replace('_', ' ')
                        if rest_resource == condition_resource:
                            # print " kw resource %s match condition resource
                            # %s" % (kw_resource, condition_resource)
            ...
            if 'pass' or 'fail' in status:
                # get the task tree and write to the OneView task file
                # Note that Wait for Task2 may be working on a list of tasks
                for task in task_list:
                    logger.info("Perf listener: retrieving task tree for %s", task['uri'])
                    task_tree = fusion_lib.fusion_api_get_task(
                        uri=task['uri'],
                        param='?view=tree')
                    write_oneview_tasks(task_json_file, task_tree, condition)
            if 'pass' in status:
                ...
                # print 'logging results and attributes'
                for attrib in attributes:
