...
class HypervisorClusterProfiles(object):
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/hypervisor-cluster-profiles%s' % (self.fusion_client._host, param)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Hypervisor Cluster Profile %s does not exist' % (name), level='WARN')
                return
