...
class ServerProfile(object):
    ...
    def create(self, body, api=None, headers=None, param=''):
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
            headers = self.fusion_client._headers.copy()
        uri = 'https://%s/rest/server-profiles%s' % (self.fusion_client._host, param)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Server Profile %s does not exist' % (name), level='WARN')
                return
