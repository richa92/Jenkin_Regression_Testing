...
class ServerHardware(object):
    ...
    def update(self, body, uri, api=None, headers=None, param=''):
        ''' use param=/environmentalConfiguration, /mpFirmwareVersion
            /powerState, /rsAHSCollection, /rsBasicCollection in keywords
        '''
        if api:
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ''' use param=/environmentalConfiguration, /isoSsoUrl,
            /remoteConsoleUrl, /utilization in keywords
        '''
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
        ...
        if uri:
            ...
            uri = 'https://%s/rest/server-hardware%s' % (self.fusion_client._host, param)
