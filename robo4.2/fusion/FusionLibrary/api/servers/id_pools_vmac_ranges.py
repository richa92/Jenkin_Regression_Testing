...
class IdPoolsVmacRange(object):
    ...
    def create(self, body, api=None, headers=None):
        if api:
            ...
            headers = self.fusion_client._headers
        uri = 'https://%s/rest/id-pools/vmac/ranges' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('VMAC range %s does not exist' % (name), level='WARN')
                return
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/id-pools/vmac/ranges%s' % (self.fusion_client._host, param)
