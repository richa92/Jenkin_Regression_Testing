...
class ServerProfileTemplate(object):
    ...
    def create(self, body, api=None, headers=None):
        ...
        uri = 'https://%s/rest/server-profile-templates' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('Server Profile Template %s does not exist' % (name), level='WARN')
                return
            ...
            # deletes all server profile templates
            uri = 'https://%s/rest/server-profile-templates' % self.fusion_client._host
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/server-profile-templates%s' % (self.fusion_client._host, param)
