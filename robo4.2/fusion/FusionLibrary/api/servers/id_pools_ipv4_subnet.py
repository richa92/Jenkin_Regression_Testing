...
class IdPoolsIpv4Subnet(object):
    ...
    def create(self, body, sessionID=None, api=None, headers=None):
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
            headers = self.fusion_client._headers
        if sessionID:
            self.fusion_client._http.headers['auth'] = sessionID
        uri = 'https://%s/rest/id-pools/ipv4/subnets/' % (self.fusion_client._host)
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('IPv4 subnet %s does not exist' % (name), level='WARN')
                return
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/id-pools/ipv4/subnets%s' % (self.fusion_client._host, param)
