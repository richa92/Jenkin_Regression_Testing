...
class FcNetwork(object):
    ...
    def _make_body_4(self, network):
        return {'type': 'fc-network',
                'linkStabilityTime': network.get('linkstime', 30),
                'autoLoginRedistribution': network.get('purpose', True),
                'name': network.get('name', "net_default"),
                'uplinkBandwidth': network.get('uplinkbw', 'Auto'),
                'connectionTemplateUri': network.get('connectionTemplateUri', None),
                'fabricType': network.get('fabricType', 'FabricAttach')}

    # X-API-Version = 101
    def _make_body_101(self, network):
        return {'type': 'fc-networkV2',
                'linkStabilityTime': network.get('linkstime', 30),
                'autoLoginRedistribution': network.get('purpose', True),
                'name': network.get('name', "net_default"),
                'connectionTemplateUri': network.get('connectionTemplateUri', None),
                'managedSanUri': network.get('managedSanUri', None),
                'fabricType': network.get('fabricType', 'FabricAttach')}
    ...
    def create(self, body, api=None, headers=None):
        ...
        uri = 'https://%s/rest/fc-networks' % (self.fusion_client._host)
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/fc-networks%s' % (self.fusion_client._host, param)
