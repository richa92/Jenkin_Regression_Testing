...
class SasLogicalInterconnect(object):
    ...
    def get(self, uri=None, api=None, headers=None, param=''):
        ...
        if uri:
            ...
            uri = 'https://%s/rest/sas-logical-interconnects%s' % (self.fusion_client._host, param)
    ...
    def post(self, body, api=None, headers=None):
        if api:
            ...
            headers = self.fusion_client._headers.copy()
        uri = 'https://%s/rest/sas-logical-interconnects' % (self.fusion_client._host)
        response = self.fusion_client.post(uri=uri, headers=headers, body=json.dumps(body))
        return response

    def put(self, body, uri, param='', api=None, headers=None):
        if api:
            headers = self.fusion_client._set_req_api_version(api=api)
        elif not headers:
            headers = self.fusion_client._headers.copy()
    ...
    def delete(self, name=None, uri=None, api=None, headers=None):
        ...
        if uri:
            ...
            if response['count'] == 0:
                logger._log('SAS Interconnect %s does not exist' % (name), level='WARN')
                return
